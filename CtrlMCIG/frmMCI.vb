...
#Region "  共通変数         "
    ...
    Private mobjLogWriterMany As clsLogWriter       'ﾃｷｽﾄﾛｸﾞｸﾗｽ(追加用)
    ...
    '↓↓↓↓↓↓************************************************************************************************************
    'SystemMate:N.Dounoshita 2013/04/23  追加用ﾛｸﾞを分ける
    'ﾛｸﾞ関係(追加用)
    Private mstrFilePathMany As String             'ﾌｧｲﾙﾊﾟｽ        (追加用)ｾｯﾄ
    Private mstrFileNameMany As String             'ﾌｧｲﾙ名         (追加用)ｾｯﾄ
    Private mintMaxDeleteDayMany As Integer        '保持日数       (追加用)ｾｯﾄ
    Private mintMaxSizeMany As Integer             '最大ﾌｧｲﾙｻｲｽﾞ   (追加用)ｾｯﾄ
    '↑↑↑↑↑↑************************************************************************************************************
    'その他
...
#End Region
#Region "  ｸﾞﾘｯﾄﾞ表示変更                   "
    Private Sub chkGridDisp_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkGridDisp.CheckedChanged
        If chkGridDisp.Checked = True Then ListBox1.Visible = True Else ListBox1.Visible = False
    End Sub
#End Region
#End Region
...
#Region "  ﾌｫｰﾑﾛｰﾄﾞ         "
    ...
    Private Sub FormLoadProcess()
        ...
        'SystemMate:N.Dounoshita 2013/04/23  追加用ﾛｸﾞを分ける
        'ﾛｸﾞ関係(追加用)
        mstrFilePathMany = TO_STRING(objConfig.GET_INFO(GKEY_LOG_FILE_PATH_MANY))                    'ﾌｧｲﾙﾊﾟｽ        (追加用)ｾｯﾄ
        mstrFileNameMany = TO_STRING(objConfig.GET_INFO(GKEY_LOG_FILE_NAME_MANY))                    'ﾌｧｲﾙ名         (追加用)ｾｯﾄ
        mintMaxDeleteDayMany = TO_NUMBER(objConfig.GET_INFO(GKEY_LOG_FILE_DELETE_DAY_MANY))          '保持日数       (追加用)ｾｯﾄ
        mintMaxSizeMany = TO_NUMBER(objConfig.GET_INFO(GKEY_LOG_FILE_SIZE_MANY))                     '最大ﾌｧｲﾙｻｲｽﾞ   (追加用)ｾｯﾄ
        '↑↑↑↑↑↑************************************************************************************************************
        'その他
        ...
        '↓↓↓↓↓↓************************************************************************************************************
        'SystemMate:N.Dounoshita 2013/04/23  追加用ﾛｸﾞを分ける
        Dim strLogFileNameMany As String = ""       'ﾛｸﾞﾌｧｲﾙ名

...
#Region "  共通変数02           "
    ...
    '共通ｵﾌﾞｼﾞｪｸﾄ
    '==================================================
    Private mobjUntMelsec As UntMelsec                      'Melsec通信ｵﾌﾞｼﾞｪｸﾄ
    Private mobjDrvSerial As DrvSerial                      'Melsec通信ｵﾌﾞｼﾞｪｸﾄ
    Private mobjDrvTCP As DrvTCP                            'Melsec通信ｵﾌﾞｼﾞｪｸﾄ
    Private mobjDrvUDP As DrvUDP                            'Melsec通信ｵﾌﾞｼﾞｪｸﾄ
    ...
    '==================================================
    '受信用COM設定
    Private mintDriver As Integer                   '通信方式 (0:RS232C 1:Ethernet)

    Private mstrCOMComName As String                '通信名
    Private mintEQ_Send02Timer As Integer           '定周期問い合わせ周期02(ms)

    Private mintCOM_TimeOut As Integer              'ﾘﾄﾗｲﾀｲﾑｱｳﾄ時間(秒)  
    Private mintCOM_Retry As Integer                'ﾘﾄﾗｲ回数
    Private mstrPortName As String                  'ﾎﾟｰﾄ名
    Private mintBaudRate As Integer                 'ﾎﾞｰﾚｰﾄ
    Private mintParity As Integer                   'ﾊﾟﾘﾃｨ(0:None 1:Odd 2:Even 3:Mark 4:Space)
    Private mintDataBits As Integer                 'ﾊﾞｲﾄﾃﾞｰﾀのﾋﾞｯﾄ数
    Private mintStopBits As Integer                 'ｽﾄｯﾌﾟﾋﾞｯﾄ数
    ...
    Private mintCodePage As Integer                 '通信に使用するﾃｷｽﾄのｺｰﾄﾞ  (932:ShiftJIS)
    Private mstrWait As String                      '伝文ｳｪｲﾄ
    Private mintSumCheck As Integer                 'ｻﾑﾁｪｯｸ有無

    Private mstrNetAddr As String                   'IPｱﾄﾞﾚｽ
    Private mintNetPort As Integer                  'ﾎﾟｰﾄ番号
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ              "
    ...
    Public Property intDebugFlag() As Integer
        ...
        Set(ByVal Value As Integer)
            mintDebugFlag = Value
            If IsNotNull(mobjUntMelsec) Then mobjUntMelsec.intDebugFlag = Value 'ｼﾘｱﾙ通信ｸﾗｽ
    ...
    Public Overridable Sub Main()
        Try
            ...
            Try
                ...
                'ｻｰﾊﾞへｿｹｯﾄ送信
                '*****************************************************
                'Debug.WriteLine("通知１" & Now.ToString)
                Call MCI_000001()
...
#Region "  初期化               "
    ...
    Public Sub Initialize()
        ...
        'Configﾌｧｲﾙ情報取得(標準部分)
        '*****************************************************
        Dim objConfig As New clsConnectConfig(CONFIG_MCIG)                              'Configﾌｧｲﾙ接続ｸﾗｽ生成
        ...
        '*****************************************************
        '受信用COM設定
        mintDriver = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_DRIVER))                '通信方式 (0:RS232C 1:TCP 2:UDP)
        mstrCOMComName = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_COMNAME))            '通信名

        mintCOM_Retry = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_RETRY))              'ﾘﾄﾗｲ回数
        mintCOM_TimeOut = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_TIMEOUT))          'ﾀｲﾑｱｳﾄ時間(秒)  
        mstrKyokuNo = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_KYOKU_NO))              '局番号
        mstrNetwkNo = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_NETWK_NO))              'ﾈｯﾄﾜｰｸ番号
        mstrPcNo = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_PC_NO))                    'PC番号
        mstrJiKyoku = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_JI_KYOKU))              '自局局番
        mstrUnitIo = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_UNIT_IO))                '要求先ﾕﾆｯﾄI/O番号
        mstrWaitTm = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_WAIT_TM))                '監視ﾀｲﾏ
        mintCodePage = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_CODE_PAGE))           '通信に使用するﾃｷｽﾄのｺｰﾄﾞ  (932:ShiftJIS)
        mstrWait = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_WAIT))                     '伝文ｳｪｲﾄ
        mintSumCheck = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_SUM_CHECK))           'ｻﾑﾁｪｯｸ有無

        mstrPortName = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_PORT_NAME))            'ﾎﾟｰﾄ名
        mintBaudRate = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_BAUDRATE))            'ﾎﾞｰﾚｰﾄ
        mintParity = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_PARITY))                'ﾊﾟﾘﾃｨ(0:None 1:Odd 2:Even 3:Mark 4:Space)
        mintDataBits = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_DATABITS))            'ﾊﾞｲﾄﾃﾞｰﾀのﾋﾞｯﾄ数
        mintStopBits = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_STOPBITS))            'ｽﾄｯﾌﾟﾋﾞｯﾄ数

        mstrNetAddr = TO_STRING(objConfig.GET_INFO(GKEY_PLC_COM_NET_ADDRESS))           'IPｱﾄﾞﾚｽ
        mintNetPort = TO_INTEGER(objConfig.GET_INFO(GKEY_PLC_COM_NET_PORT))             'ﾎﾟｰﾄ番号
        ...
        '' '' '' ''mobjPLC.Open()                                              '通信確立
        If mintDriver = UntMelsec.DriverType.Serial Then
            mobjDrvSerial = New DrvSerial(mobjOwner, _
                                          mstrPortName, _
                                          mintBaudRate, _
                                          mintParity, _
                                          mintDataBits, _
                                          mintStopBits, _
                                          mintCodePage)             'ｵﾌﾞｼﾞｪｸﾄ生成
            mobjUntMelsec = New UntMelsec(mobjDrvSerial)
            mobjUntMelsec.TimeOut = mintCOM_TimeOut
            mobjUntMelsec.Retry = mintCOM_Retry
            mobjUntMelsec.Wait = mstrWait
            mobjUntMelsec.SumCheck = mintSumCheck
            mobjUntMelsec.strKyokuNo = mstrKyokuNo                  '局番号
            mobjUntMelsec.strNetwkNo = mstrNetwkNo                  'ﾈｯﾄﾜｰｸ番号
            mobjUntMelsec.strPcNo = mstrPcNo                        'PC番号
            mobjUntMelsec.strJiKyoku = mstrJiKyoku                  '自局局番
            mobjUntMelsec.strUnitIo = mstrUnitIo                    '要求先ﾕﾆｯﾄI/O番号
            mobjUntMelsec.strWaitTm = mstrWaitTm                    '監視ﾀｲﾏ
            Call mobjDrvSerial.Connect()
        ElseIf mintDriver = UntMelsec.DriverType.TCP Then
            mobjDrvTCP = New DrvTCP(mobjOwner, _
                                    mstrNetAddr, _
                                    mintNetPort)                    'ｵﾌﾞｼﾞｪｸﾄ生成
            mobjUntMelsec = New UntMelsec(mobjDrvTCP)
            mobjUntMelsec.TimeOut = mintCOM_TimeOut
            mobjUntMelsec.Retry = mintCOM_Retry
            mobjUntMelsec.Wait = mstrWait
            mobjUntMelsec.SumCheck = mintSumCheck
            ...
            mobjUntMelsec.strUnitIo = mstrUnitIo                    '要求先ﾕﾆｯﾄI/O番号
            mobjUntMelsec.strWaitTm = mstrWaitTm                    '監視ﾀｲﾏ
            Call mobjDrvTCP.Connect()
        ElseIf mintDriver = UntMelsec.DriverType.UDP Then
            mobjDrvUDP = New DrvUDP(mobjOwner, _
                                    mstrNetAddr, _
                                    mintNetPort)                    'ｵﾌﾞｼﾞｪｸﾄ生成
            mobjUntMelsec = New UntMelsec(mobjDrvUDP)
            mobjUntMelsec.TimeOut = mintCOM_TimeOut
            mobjUntMelsec.Retry = mintCOM_Retry
            mobjUntMelsec.Wait = mstrWait
            mobjUntMelsec.SumCheck = mintSumCheck
            ...
            Call mobjDrvTCP.Connect()
        Else
            Throw New Exception(ERRMSG_DISP_PARAM & "Configﾌｧｲﾙ定義 通信方式異常")
        End If
        ...
        Try
            ...
            If mstrSvrSockSendFlag = FLAG_ON Then
                ...
                If mblnSockSend = True Then
                    ...
                    'Debug.WriteLine("通知" & Now.ToString)
                Else
                    'Debug.WriteLine("スルー" & Now.ToString)
                End If
...
#Region "  通信ｸﾛｰｽﾞ                        "
    ...
    Public Sub Close()
        Try
            ...
            '****************************************************
            If mintDriver = UntMelsec.DriverType.Serial Then
                Call mobjDrvSerial.Close()
            ElseIf mintDriver = UntMelsec.DriverType.TCP Then
                Call mobjDrvTCP.Close()
            ElseIf mintDriver = UntMelsec.DriverType.UDP Then
                Call mobjDrvTCP.Close()
            End If
...
#Region "  ﾚｼﾞｽﾀ読込み(ﾜｰﾄﾞ単位)"
    ...
    ''' <remarks></remarks>
    '''*******************************************************************************************************************
    Public Function SeqWordReadCmndSend(ByVal intAdrs As Integer _
                                      , ByVal intSize As Integer _
                                      , ByRef intData() As Integer _
                                       ) As DrvBase.RetCode
        Dim intReturn As CtrlPlcAccess.DrvBase.RetCode = CtrlPlcAccess.DrvBase.RetCode.NG                   '関数戻値


        '***************************************
        'ﾚｼﾞｽﾀ読込
        '***************************************
        intReturn = mobjUntMelsec.SeqWordReadCmndSend(intAdrs, intSize, intData)           '配列取得
...
#Region "  ﾚｼﾞｽﾀ書込み(ﾜｰﾄﾞ単位)"
    ...
    ''' <remarks></remarks>
    '''*******************************************************************************************************************
    Public Function SeqWordWriteCmndSend(ByVal intAdrs As Integer _
                                       , ByVal intData() As Integer _
                                        ) As DrvBase.RetCode
        Dim intReturn As CtrlPlcAccess.DrvBase.RetCode = RetCode.NG                   '関数戻値


        '***************************************
        'ﾚｼﾞｽﾀ書込
        '***************************************
        intReturn = mobjUntMelsec.SeqWordWriteCmndSend(intAdrs, intData)               '配列取得
                ...
                If blnOnlineBefore <> mblnOnline Then
                    ...
                    If mintDebugFlag = FLAG_OFF Then
                        For ii As Integer = REG_START To REG_MAX
                            ...
                            'Dim strValue2 As String = Convert.ToString(mintNRegD(ii), 2)

                            'If strValue2 = "11111111111111111001100110011001" Then
                            '    Stop
                            'End If

                            ''2017/08/01 Mod infomate ITF対応
                            Dim chkKeta As Integer = 16
                            Select Case mstrFEQ_ID
                                Case FEQ_ID_JSYS0004, FEQ_ID_JSYS0006
                                    Select Case ii
                                        Case 41361 To 41368
                                            chkKeta = 32
                                        Case Else
                            ...
                            Dim strTemp As String = Change10To2(mintNRegD(ii), chkKeta, ii)
            ...
            If IsNothing(intPartBit) = False Then
                ...
                If Len(strTemp) < 16 Then
                    ...
                    For ii = 1 To 16 - Len(strTemp)
                        strTemp = "0" & strTemp
                    Next
                ElseIf Len(strTemp) > 16 Then
                    strTemp = Right(strTemp, 16)
                End If
...
#Region "  ﾚｼﾞｽﾀ配列ﾃﾞｰﾀ書込み                      "
    ...
    Public Sub WriteRegister(ByVal strAddress As String _
        ...
        Try
            ...
            'ﾚｼﾞｽﾀ書込
            '***************************************
            Dim intRetPLC As CtrlPlcAccess.DrvBase.RetCode
            intRetPLC = mobjUntMelsec.SeqWordWriteCmndSend(intPartAddress, intTemp02)               '配列取得
            If intRetPLC <> CtrlPlcAccess.DrvBase.RetCode.OK Then
                '(失敗した場合)
                Throw New Exception(MCIC_MSG_1007 _
                                    & "[局番号:" & TO_STRING(mstrKyokuNo) & "]" _
                                    & "[ﾈｯﾄﾜｰｸ番号:" & TO_STRING(mstrNetwkNo) & "]" _
                                    & "[PC番号:" & TO_STRING(mstrPcNo) & "]" _
                                    & "[自局局番:" & TO_STRING(mstrJiKyoku) & "]" _
...
#Region "  読込ﾚｼﾞｽﾀ配列更新            (対象となる全ﾚｼﾞｽﾀ)     "
    ...
    Private Sub UpdateReadArray()
        Try
            ...
            'Dim intRet As CtrlPlcAccess.DrvBase.RetCode      '戻り値
            ...
            Select Case mstrFEQ_ID
                ...
                Case FEQ_ID_JSYS0003, FEQ_ID_JSYS0004, FEQ_ID_JSYS0006
                    '↓↓↓↓↓↓************************************************************************************************************
...
#Region "  読込ﾚｼﾞｽﾀ配列更新            (指定したﾚｼﾞｽﾀ)         "
    ...
    Private Sub UpdateReadPartArray(ByVal intAdrs As Integer _
        ...
        Dim intTemp(intSize) As Integer                  'ﾚｼﾞｽﾀ値一時保管
        Dim intRet As CtrlPlcAccess.DrvBase.RetCode      '戻り値
        ...
        intRet = mobjUntMelsec.SeqWordReadCmndSend(intAdrs, intSize, intTemp)        '配列取得
        If intRet = CtrlPlcAccess.DrvBase.RetCode.OK Then
            '(成功の場合)
...
#Region "  ID:0101      ﾚｼｽﾞﾀ書込要求(ﾜｰﾄﾞ対応)             "
    ...
    Private Function ID0101Process(ByRef objTLIF_TRNS_SEND As TBL_TLIF_TRNS_SEND) As RetCode
        Try
            ...
            '***************************************
            Dim intRetPLC As CtrlPlcAccess.DrvBase.RetCode
            intRetPLC = mobjUntMelsec.SeqWordWriteCmndSend(intPartAddress, intAryData)          '配列取得
            If intRetPLC <> CtrlPlcAccess.DrvBase.RetCode.OK Then
                '(失敗した場合)
                Throw New Exception(MCIC_MSG_1007 _
                                    & "[局番号:" & TO_STRING(mstrKyokuNo) & "]" _

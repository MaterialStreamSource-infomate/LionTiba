...
#Region "  共通変数　                               "
    ...
    Private mstrFTRK_BUF_NO_Disp As String              '出庫ST
    ...
    Private mstrXPLAN_NUM As String                     '計画PL数
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義　                            "
    ...
    '''''' =======================================
    '''''' <summary>計画PL数</summary>
    '''''' <remarks></remarks>
    '''''' =======================================
    ' ''Public Property userPLAN_NUM() As String
    ' ''    Get
    ' ''        Return mstrPLAN_NUM
    ' ''    End Get
    ' ''    Set(ByVal value As String)
    ...
    Private Sub cboFHINMEI_CD_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboFHINMEI_CD.TextChanged
        ...
        If mFlag_Form_Load = False Then
            ...
            Else
                ...
                mstrXPROD_LINE = TO_STRING(objDataSet.Tables("WRAPPING_OUT").Rows(0).Item("XPROD_LINE"))
...
#Region "  ﾌｫｰﾑﾛｰﾄﾞ     処理                        "
    ...
    Private Sub LoadProcess()
        ...
        strSQL &= vbCrLf & "    AND XSTS_WRAPPING_MATERIAL.FTRK_BUF_NO = '" & mstrFTRK_BUF_NO & "' "
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            For Each objRow In objDataSet.Tables(strDataSetName).Rows
                ...
                ' ﾃﾞｰﾀ取得
                '********************************************************************
                mstrFTRK_BUF_NO_Disp = TO_STRING(objRow("FBUF_NAME"))
                ...
                mstrXPLAN_NUM = TO_STRING(objRow("XPLAN_NUM"))
        ...
        '出庫ST ｾｯﾄ
        '===================================
        lblOUT_ST.Text = mstrFTRK_BUF_NO_Disp
        ...
        ElseIf mintFPROGRESS = XPROGRESS_START Then
            ...
            '計画PL数  ｾｯﾄ
            '===================================
            txtPL_VOL.Enabled = False
            txtPL_VOL.Text = mstrXPLAN_NUM
        ...
        Select Case udtCheck_Case
            Case menmCheckCase.Out_SetBtn_Click
                ...
                If txtPL_VOL.Text = "" Then
                    '（入力無しの場合）
                    Call gobjComFuncFRM.DisplayPopup("計画PL数を入力してください。", _
                                      PopupFormType.Ok, _
                                      PopupIconType.Information)
                    Exit Select
                End If
...
#Region "  ｿｹｯﾄ送信(出庫登録)                   "
    ...
    Private Function SendSocket01() As Boolean
        ...
        objTelegram.FORMAT_ID = FORMAT_DSP_PRI & FSYORI_ID_J400201    'ﾌｫｰﾏｯﾄ名ｾｯﾄ
        ...
        objTelegram.SETIN_DATA("XDSPPLAN_NUM", TO_STRING(txtPL_VOL.Text))                   '計画数量
...
#Region "  ｿｹｯﾄ送信(登録解除)                   "
    ...
    Private Function SendSocket02() As Boolean
        ...
        objTelegram.SETIN_DATA("XDSPPLAN_NUM", "0")                                         '計画数量
        objTelegram.SETIN_DATA("XDSPRESULT_NUM", "0")                                       '実績数量

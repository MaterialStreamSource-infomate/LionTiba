...
#Region "  F6(登録)         ﾎﾞﾀﾝ押下処理　          "
    ...
    Public Overrides Sub F6Process()
        ...
        'ｿｹｯﾄ送信処理
        '********************************************************************
        If (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8000) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8001) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8002) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8100) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8101) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8102) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8103) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8104) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8105) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8106) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8107) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8108) Or _
           (mstrFTRK_BUF_NO = FTRK_BUF_NO_J8109) Then
            '(ﾊﾞﾗ平置き、平置き、外部倉庫の時)

            If SendSocket02_HIRA() = False Then
                Exit Sub
            End If
        ...
        Else
            ...
            If SendSocket02_ASOUKO() = False Then
                Exit Sub
            End If
            ...
            Case menmCheckCase.Reg_Click
                ...
                For ii As Integer = 0 To grdList.Rows.Count - 1
                    ...
                    If grdList.Item(menmListCol.SEL_FLG, ii).Value = mstrSEL_ON Then
                        ...
                        If blnSEL = False Then
                            '(初回)
                            strSQL &= vbCrLf & "  '" & grdList.Item(menmListCol.FPALLET_ID, ii).Value & "' "
                        Else
                            '(以降)
                            strSQL &= vbCrLf & ", '" & grdList.Item(menmListCol.FPALLET_ID, ii).Value & "' "
                        End If
                ...
                If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
                    '(見つかった場合)
                    Call gobjComFuncFRM.DisplayPopup(FRM_MSG_FRM308030_09, PopupFormType.Ok, PopupIconType.Information)
                    blnCheckErr = True
                    Exit Select
                End If
                ...
                '    strSQL &= vbCrLf & " WHERE 1 = 1 "
                '    strSQL &= vbCrLf & "   AND XRST_OUT.XSYUKKA_NO        = '" & grdList.Item(menmListCol.XSYUKKA_NO, ii).Value & "' "
                '    strSQL &= vbCrLf & "   AND XRST_OUT.XPLN_DTL_NO       = '" & grdList.Item(menmListCol.XPLN_DTL_NO, ii).Value & "' "
                '    strSQL &= vbCrLf & "   AND XRST_OUT.FTRK_BUF_NO       =  " & grdList.Item(menmListCol.FTRK_BUF_NO, ii).Value
                '    strSQL &= vbCrLf & "   AND XRST_OUT.XPALLET_ID_CHECK  = '" & grdList.Item(menmListCol.FPALLET_ID, ii).Value & "' "
                '    'strSQL &= vbCrLf & "   AND XRST_OUT.XPALLET_ID_KARI   = XRST_OUT.XPALLET_ID_CHECK "
                '    '↓↓↓↓ 2012.11.26 11:20 H.Morita
                '    strSQL &= vbCrLf & "   AND XRST_OUT.XSEIZOU_DT        = TO_DATE('" & grdList.Item(menmListCol.XSEIZOU_DT, ii).Value & "', 'YYYY/MM/DD HH24:MI:SS')"
                '    '↑↑↑↑ 2012.11.26 11:20 H.Morita
                ...
                '    If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
                '        '(見つかった場合)
                '        Call gobjComFuncFRM.DisplayPopup(FRM_MSG_FRM308030_10, PopupFormType.Ok, PopupIconType.Information)
                '        blnCheckErr = True
                '        Exit Select
                '    End If
...
#Region "　構造体ｾｯﾄ　                              "
    ...
    Private Sub SearchItem_Set()
        ...
        '↓↓↓↓ 2012.11.30 10:30 H.Morita 同一出荷№+異なる出荷明細№+同一品目の時の検品対応
        mudtSEARCH_ITEM.XPLN_DTL_NO = mstrXPLN_DTL_NO                       '計画明細№
        '↑↑↑↑ 2012.11.30 10:30 H.Morita 同一出荷№+異なる出荷明細№+同一品目の時の検品対応
...
#Region "　ｸﾞﾘｯﾄﾞ表示　                             "
    ...
    Public Overrides Sub grdListDisplayChild()
        ...
        strSQL &= vbCrLf & "      WHEN " & FTRK_BUF_NO_J8201 & " THEN TRST_STOCK.FTR_VOL "
        strSQL &= vbCrLf & "      WHEN " & FTRK_BUF_NO_J8202 & " THEN TRST_STOCK.FTR_VOL "
        strSQL &= vbCrLf & "      WHEN " & FTRK_BUF_NO_J8203 & " THEN TRST_STOCK.FTR_VOL "
        strSQL &= vbCrLf & "      WHEN " & FTRK_BUF_NO_J8204 & " THEN TRST_STOCK.FTR_VOL "
        strSQL &= vbCrLf & "      WHEN " & FTRK_BUF_NO_J8205 & " THEN TRST_STOCK.FTR_VOL "
        strSQL &= vbCrLf & "      WHEN " & FTRK_BUF_NO_J8206 & " THEN TRST_STOCK.FTR_VOL "
        strSQL &= vbCrLf & "      WHEN " & FTRK_BUF_NO_J8207 & " THEN TRST_STOCK.FTR_VOL "
        strSQL &= vbCrLf & "      WHEN " & FTRK_BUF_NO_J8208 & " THEN TRST_STOCK.FTR_VOL "
        strSQL &= vbCrLf & "      ELSE (TRST_STOCK.FTR_VOL - TRST_STOCK.FTR_RES_VOL) "
        strSQL &= vbCrLf & "     END) AS XTUMIKOMI_SU "                                         '積載数
        '↑↑↑ 2012/11/09 11:55
        strSQL &= vbCrLf & "  , 0 AS XKENPIN_SU "                                               '検品数
        ...
        '計画明細№
        '----------------------------------------------
        If mudtSEARCH_ITEM.XPLN_DTL_NO <> CBO_ALL_VALUE Then
            '(全検索以外の場合)
            strSQL &= vbCrLf & "      AND XPLN_OUT_DTL.XPLN_DTL_NO = " & mudtSEARCH_ITEM.XPLN_DTL_NO
        End If
        '↑↑↑↑ 2012.11.30 10:30 H.Morita 同一出荷№+異なる出荷明細№+同一品目の時の検品対応
        ...
        '↓↓↓↓ 2012.11.30 10:30 H.Morita 同一出荷№+異なる出荷明細№+同一品目の時の検品対応
        strSQL &= vbCrLf & "   AND XRST_OUT.XPLN_DTL_NO    = '" & mudtSEARCH_ITEM.XPLN_DTL_NO & "' "      '計画明細№
        '↑↑↑↑ 2012.11.30 10:30 H.Morita 同一出荷№+異なる出荷明細№+同一品目の時の検品対応
        ...
        strSQL &= vbCrLf & "   AND XRST_OUT.XKENPIN_FLAG   = " & XKENPIN_FLAG_JOFF                        '検品ﾌﾗｸﾞ = 未検品
...
#Region "　ｸﾞﾘｯﾄﾞ表示設定　                         "
    ...
    Private Sub grdListSetup()
        ...
        If (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8000) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8001) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8002) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8100) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8101) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8102) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8103) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8104) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8105) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8106) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8107) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8108) Or _
           (TO_INTEGER(mstrFTRK_BUF_NO) = FTRK_BUF_NO_J8109) Then
            '(ﾊﾞﾗ平置き、1F平置き、3F平置き、外部倉庫の時)
        Else
            '(平置き関係以外(ﾊﾞｰｽの時)の時)
...
#Region "  ｿｹｯﾄ送信02(自動倉庫)                   　"
    ...
    '【戻値】
    '*******************************************************************************************************************
    Private Function SendSocket02_ASOUKO() As Boolean
    ...
    End Function
#End Region
#Region "  ｿｹｯﾄ送信02(平置き、外部倉庫)           　"
    '*******************************************************************************************************************
    '【機能】同上
    '【引数】
    '【戻値】
    '*******************************************************************************************************************
    Private Function SendSocket02_HIRA() As Boolean
        ...
        For ii As Integer = 0 To grdList.Rows.Count - 1
            ...
            strDSPST_FM = TO_STRING(grdList2.Item(menmListCol2.FTRK_BUF_NO_BERTH, 0).Value) '搬送元STﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｬ№
            ...
            'ﾃﾞｰﾀ
            strXDSPPALLET_ID_KARI = TO_STRING(grdList2.Item(menmListCol2.FPALLET_ID, 0).Value)          'ﾊﾟﾚｯﾄID(仮引当)

            'strDSPTR_VOL = TO_STRING(TO_DECIMAL(grdList2.Item(menmListCol2.XKENPIN_VOL, 0).Value))      '搬送管理量
            strDSPTR_VOL = TO_STRING(TO_DECIMAL(grdList.Item(menmListCol.XKENPIN_SU, ii).Value))         '検品数

            strXDSPSYUKKA_NO = TO_STRING(grdList2.Item(menmListCol2.XSYUKKA_NO, 0).Value)               '出荷№
            strXDSPPLN_DTL_NO = TO_STRING(grdList2.Item(menmListCol2.XPLN_DTL_NO, 0).Value)             '計画明細№

            'strXDSPPALLET_ID_CHECK = TO_STRING(grdList2.Item(menmListCol2.XKENPIN_PALLET_ID, 0).Value)  'ﾊﾟﾚｯﾄID(検品済)
            strXDSPPALLET_ID_CHECK = TO_STRING(grdList.Item(menmListCol.FPALLET_ID, ii).Value)           'ﾊﾟﾚｯﾄID(検品済)

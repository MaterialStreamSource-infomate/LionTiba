...
#Region "  F6(登録)     ﾎﾞﾀﾝ押下処理　              "
    ...
    Public Overrides Sub F6Process()
        ...
        'ｵﾌﾞｼﾞｪｸﾄNothingﾁｪｯｸ
        If IsNull(gobjFRM_305031) = False Then
            gobjFRM_305031.Close()
            gobjFRM_305031.Dispose()
            gobjFRM_305031 = Nothing
        End If

        '********************************************************************
        ' 子画面展開
        '********************************************************************
        gobjFRM_305031 = New FRM_305031                             'ｵﾌﾞｼﾞｪｸﾄ作成

        Call SetProperty(gobjFRM_305031)                  'ﾌﾟﾛﾊﾟﾃｨｾｯﾄ
        Call gobjFRM_305031.ShowDialog()        '画面表示
    ...
    Private Function InputCheck(ByVal udtCheck_Case As menmCheckCase) As Boolean
        ...
        Select Case udtCheck_Case
            Case menmCheckCase.SearchBtn_Click
                ...
                '    '(ﾗｲﾝ№が選択されていない場合)
                '    Call gobjComFuncFRM.DisplayPopup(FRM_MSG_FRM305030_01, PopupFormType.Ok, PopupIconType.Information)
                '    blnCheckErr = True
                '    Exit Select
                'End If
                ...
                If (cboXPALLET_NO_FROM.Text <> "") And _
                   (cboXPALLET_NO_TO.Text <> "") Then
                    If (TO_STRING(cboXPALLET_NO_FROM.SelectedValue.ToString) > TO_STRING(cboXPALLET_NO_TO.SelectedValue.ToString)) Then
                        '(ﾊﾟﾚｯﾄ№の大小が間違っている場合)
                        Call gobjComFuncFRM.DisplayPopup(FRM_MSG_FRM305030_02, PopupFormType.Ok, PopupIconType.Information)
                        blnCheckErr = True
                        Exit Select
                    End If
                End If
...
#Region "　ｸﾞﾘｯﾄﾞ表示　                             "
    ...
    Public Overrides Sub grdListDisplayChild()
        ...
        strSQL &= vbCrLf & "  , HASH02.FGAMEN_DISP AS XSYUKKA_KAHI_DSP "    '在庫情報.                  出荷可否(表示用)
        ...
        strSQL &= vbCrLf & "  , TRST_STOCK.XHORYU_NO "                      '在庫情報.                  保留NO
        strSQL &= vbCrLf & "  , TRST_STOCK.XHORYU_REASON "                  '在庫情報.                  保留理由
        strSQL &= vbCrLf & "  , TPRG_TRK_BUF.FDISP_ADDRESS "                'ﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｬ.              表記用ｱﾄﾞﾚｽ(ﾛｹｰｼｮﾝ)
        strSQL &= vbCrLf & "  , TRST_STOCK.XSTRETCH_STS "                   '在庫情報.          (非表示)ｽﾄﾚｯﾁｽﾃｰﾀｽ
        strSQL &= vbCrLf & "  , HASH04.FGAMEN_DISP AS XSTRETCH_STS_DSP "    '在庫情報.                  ｽﾄﾚｯﾁｽﾃｰﾀｽ(表示用)
        strSQL &= vbCrLf & "  , TRST_STOCK.XKENSA_KUBUN "                   '在庫情報.          (非表示)検索区分
        strSQL &= vbCrLf & "  , HASH05.FGAMEN_DISP AS XKENSA_KUBUN_DSP "    '在庫情報.                  検索区分(表示用)
        strSQL &= vbCrLf & "  , TRST_STOCK.XAB_KUBUN "                      '在庫情報.                  AB区分
        strSQL &= vbCrLf & "  , TRST_STOCK.FARRIVE_DT "                     '在庫情報.                  在庫発生日時
        strSQL &= vbCrLf & "  , TPRG_TRK_BUF.FBUF_IN_DT "                   'ﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｬ.              ﾊﾞｯﾌｬ入日時
        ...
        strSQL &= vbCrLf & "  , " & gobjComFuncFRM.GetSQLHashTableFrom01("HASH02", "TRST_STOCK", "XSYUKKA_KAHI")
        ...
        strSQL &= vbCrLf & "    AND " & gobjComFuncFRM.GetSQLHashTableWhere01("HASH02", "TRST_STOCK", "XSYUKKA_KAHI")
...
#Region "　ｸﾞﾘｯﾄﾞ表示設定　                         "
    ...
    Private Sub grdListSetup()
        ...
        lblPALLET_SU.Text = ""          'ﾊﾟﾚｯﾄ数
        lblKONSU_GOUKEI.Text = ""       '梱数合計
...
#Region "　ﾌﾟﾛﾊﾟﾃｨｾｯﾄ(詳細)　                       "
    ...
    '【戻値】
    '*******************************************************************************************************************
    Private Sub SetProperty(ByVal objForm As FRM_305031)
...
#Region "  ﾊﾟﾚｯﾄ数、梱数合計 表示処理               "
    '*******************************************************************************************************************
    '【機能】同上
    '【引数】
    '【戻値】
    '*******************************************************************************************************************
    Private Sub Display_PALLET_SU()
        ...
        strSQL = ""
        strSQL &= vbCrLf & "SELECT "
        strSQL &= vbCrLf & "    COUNT(0) AS PALLET_SU "                      'ﾊﾟﾚｯﾄ数
        strSQL &= vbCrLf & "  , SUM(TRST_STOCK.FTR_VOL) AS KONSU_GOUKEI "    '梱数合計
        strSQL &= vbCrLf & " FROM "
        strSQL &= vbCrLf & "    TPRG_TRK_BUF "                          'ﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｧ
        strSQL &= vbCrLf & "  , TRST_STOCK "                            '在庫情報
        strSQL &= vbCrLf & "  , TMST_ITEM "                             '品目ﾏｽﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            lblPALLET_SU.Text = TO_STRING(TO_INTEGER(objRow("PALLET_SU")))
            lblKONSU_GOUKEI.Text = TO_STRING(TO_INTEGER(objRow("KONSU_GOUKEI")))

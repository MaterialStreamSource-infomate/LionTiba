...
#Region "　共通変数　                           "
    ...
    Enum menmListCol
        ...
        FMSG_NAIYOU             'ﾒｯｾｰｼﾞﾏｽﾀ.ﾒｯｾｰｼﾞ内容
...
#Region "　ｸﾞﾘｯﾄﾞ表示　                         "
    ...
    Public Overrides Sub grdListDisplayChild()
        ...
        strSQL &= vbCrLf & "    TLOG_MESSAGE.FHASSEI_DT "                                             'ﾒｯｾｰｼﾞﾛｸﾞ.     発生日時　 
        strSQL &= vbCrLf & "  , NVL(TMST_MESSAGE.FMSG_NAIYOU,TLOG_MESSAGE.FMSG_ID) AS FMSG_NAIYOU "   'ﾒｯｾｰｼﾞﾏｽﾀ.     ﾒｯｾｰｼﾞ
        strSQL &= vbCrLf & "  , TLOG_MESSAGE.FMSG_PRM1 "                                              'ﾒｯｾｰｼﾞﾛｸﾞ.     ﾊﾟﾗﾒｰﾀ1
        strSQL &= vbCrLf & "  , TLOG_MESSAGE.FMSG_PRM2 "                                              'ﾒｯｾｰｼﾞﾛｸﾞ.     ﾊﾟﾗﾒｰﾀ2　
        strSQL &= vbCrLf & "  , TLOG_MESSAGE.FMSG_PRM3 "                                              'ﾒｯｾｰｼﾞﾛｸﾞ.     ﾊﾟﾗﾒｰﾀ3
        strSQL &= vbCrLf & "  , TLOG_MESSAGE.FMSG_PRM4 "                                              'ﾒｯｾｰｼﾞﾛｸﾞ.     ﾊﾟﾗﾒｰﾀ4
        strSQL &= vbCrLf & "  , TLOG_MESSAGE.FMSG_PRM5 "                                              'ﾒｯｾｰｼﾞﾛｸﾞ.     ﾊﾟﾗﾒｰﾀ5
        strSQL &= vbCrLf & "  , TLOG_MESSAGE.FLOG_NO "                                                'ﾒｯｾｰｼﾞﾛｸﾞ.     ﾛｸﾞ№
        strSQL &= vbCrLf & " FROM "
        ...
        strSQL &= vbCrLf & "    AND TLOG_MESSAGE.FMSG_ID = TMST_MESSAGE.FMSG_ID(+) "        'ﾒｯｾｰｼﾞﾛｸﾞとﾒｯｾｰｼﾞﾏｽﾀのﾒｯｾｰｼﾞIDを外部結合

        '----------------------------
        'ﾛｸﾞ発生日時
        '----------------------------
        If mudtSEARCH_ITEM.KIKAN_FROM <> "" Then
            strSQL &= vbCrLf & "    AND TLOG_MESSAGE.FHASSEI_DT >= TO_DATE('" & mudtSEARCH_ITEM.KIKAN_FROM & "','YYYY/MM/DD HH24:MI:SS')"          'ｼｽﾃﾑｴﾗｰﾛｸﾞ.     発生日時
        End If
        If mudtSEARCH_ITEM.KIKAN_TO <> "" Then
            strSQL &= vbCrLf & "    AND TLOG_MESSAGE.FHASSEI_DT <= TO_DATE('" & mudtSEARCH_ITEM.KIKAN_TO & "','YYYY/MM/DD HH24:MI:SS')"            'ｼｽﾃﾑｴﾗｰﾛｸﾞ.     発生日時
        End If

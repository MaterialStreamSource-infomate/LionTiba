...
#Region "　共通変数　                           "
    ...
    Enum menmListCol
        ...
        FHINMEI                     '品目ﾏｽﾀ.			    品名
        FIN_KUBUN                   '生産ﾗｲﾝﾏｽﾀ.            入庫区分
        FIN_KUBUN_DSP               '生産ﾗｲﾝﾏｽﾀ.            入庫区分(表示用)
...
#Region "  F4(追加)　ﾎﾞﾀﾝ押下処理　             "
    ...
    Public Overrides Sub F4Process()
        ...
        ' ﾌﾟﾛﾊﾟﾃｨｾｯﾄ
        '************************************
        If IsNull(gobjFRM_306061) = False Then
            gobjFRM_306061.Close()
            gobjFRM_306061.Dispose()
            gobjFRM_306061 = Nothing
        End If
        gobjFRM_306061 = New FRM_306061
        Call SetProperty(BUTTONMODE_ADD)                   'ﾌﾟﾛﾊﾟﾃｨｾｯﾄ
        gobjFRM_306061.userButtonMode = BUTTONMODE_ADD     'ﾎﾞﾀﾝ

        Dim intRet As DialogResult
        intRet = gobjFRM_306061.ShowDialog()            '展開
...
#Region "  F5(変更)　ﾎﾞﾀﾝ押下処理　             "
    ...
    Public Overrides Sub F5Process()
        ...
        gobjFRM_306061.userButtonMode = BUTTONMODE_UPDATE     'ﾎﾞﾀﾝ
...
#Region "  F6(削除)　ﾎﾞﾀﾝ押下処理　             "
    ...
    Public Overrides Sub F6Process()
        ...
        gobjFRM_306061.userButtonMode = BUTTONMODE_DELETE     'ﾎﾞﾀﾝ
...
#Region "　ｸﾞﾘｯﾄﾞ表示　                         "
    ...
    Public Overrides Sub grdListDisplayChild()
        ...
        strSQL &= vbCrLf & "     XMST_PROD_LINE.XPROD_LINE "                            '生産ﾗｲﾝﾏｽﾀ.		生産ﾗｲﾝ№
        strSQL &= vbCrLf & "   , XMST_PROD_LINE.XPROD_LINE_NAME "                       '生産ﾗｲﾝﾏｽﾀ.        生産ﾗｲﾝ名称
        strSQL &= vbCrLf & "   , XMST_PROD_LINE.FHINMEI_CD "                            '生産ﾗｲﾝﾏｽﾀ.        品目ｺｰﾄﾞ
        strSQL &= vbCrLf & "   , TMST_ITEM.FHINMEI "                                    '品目ﾏｽﾀ.           品名
        strSQL &= vbCrLf & "   , XMST_PROD_LINE.FIN_KUBUN "                             '生産ﾗｲﾝﾏｽﾀ.        入庫区分
        strSQL &= vbCrLf & "   , HASH01.FGAMEN_DISP AS FIN_KUBUN_DSP "                  '生産ﾗｲﾝﾏｽﾀ.        入庫区分(表示用)
        ...
        strSQL &= vbCrLf & "   , " & gobjComFuncFRM.GetSQLHashTableFrom01("HASH01", "XMST_PROD_LINE", "FIN_KUBUN")     '入庫区分
        ...
        strSQL &= vbCrLf & "   AND XMST_PROD_LINE.FHINMEI_CD  = TMST_ITEM.FHINMEI_CD(+) "
        strSQL &= vbCrLf & "   AND " & gobjComFuncFRM.GetSQLHashTableWhere01("HASH01", "XMST_PROD_LINE", "FIN_KUBUN")  '入庫区分
...
#Region "  ﾒﾝﾃﾅﾝｽ入力画面へﾌﾟﾛﾊﾟﾃｨｾｯﾄ　         "
    ...
    Private Sub SetProperty(ByVal intBtnMode As Integer)
        ...
        If intBtnMode = BUTTONMODE_ADD Then
            '(追加のとき)
            gobjFRM_306061.userXPROD_LINE = Nothing             '生産ﾗｲﾝ№

        ElseIf intBtnMode = BUTTONMODE_UPDATE Then
            '(変更のとき)
            intRow = grdList.SelectedRows(0).Index
            gobjFRM_306061.userXPROD_LINE = TO_STRING(grdList.Item(menmListCol.XPROD_LINE, intRow).Value)     '生産ﾗｲﾝ№

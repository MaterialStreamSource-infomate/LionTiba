...
#Region "　ﾌｫｰﾑﾛｰﾄﾞ                                 "
    ...
    Public Overrides Sub InitializeChild()
        ...
        ' ST             ｾｯﾄ
        '**********************************************************
        cboFBUF_NAME.Items.Clear()
        cboFBUF_NAME.Items.Add("C01-C04")
        cboFBUF_NAME.Items.Add("D45")
        cboFBUF_NAME.Items.Add("D81-D85")
        cboFBUF_NAME.SelectedIndex = 0
...
#Region "  F2(出庫登録)  ﾎﾞﾀﾝ押下処理　             "
    ...
    Public Overrides Sub F2Process()
        ...
        ' ﾌﾟﾛﾊﾟﾃｨｾｯﾄ
        '************************************
        If IsNull(gobjFRM_310051) = False Then
            gobjFRM_310051.Close()
            gobjFRM_310051.Dispose()
            gobjFRM_310051 = Nothing
        End If

        gobjFRM_310051 = New FRM_310051

        Call SetProperty()      'ﾌﾟﾛﾊﾟﾃｨｾｯﾄ

        Dim intRet As DialogResult
        intRet = gobjFRM_310051.ShowDialog()            '展開
...
#Region "  包材出庫登録画面へﾌﾟﾛﾊﾟﾃｨｾｯﾄ　           "
    ...
    Private Sub SetProperty()
        ...
        '↓↓↓↓↓↓************************************************************************************************************
        'JobMate:IKEDA 2017/07/06 生産入庫登録コンボボックス対応 ↓↓↓↓↓↓
        gobjFRM_310051.intCboIndex = cboFBUF_NAME.SelectedIndex
        'JobMate:IKEDA 2017/07/06 生産入庫登録コンボボックス対応 ↑↑↑↑↑↑
        '↑↑↑↑↑↑************************************************************************************************************

        '共通ﾃﾞｰﾀ
        gobjFRM_310051.userXPROD_LINE = TO_STRING(grdList.Item(menmListCol.PROD_LINE, grdList.SelectedRows(0).Index).Value)                     '生産ﾗｲﾝNo.
...
#Region "　ｸﾞﾘｯﾄﾞ表示　                             "
    ...
    Public Overrides Sub grdListDisplayChild()
        ...
        strSQL &= vbCrLf & "       XSTS_WRAPPING_MATERIAL_1F.XPROD_LINE"               '包材出庫設定状態(1F)      .生産ﾗｲﾝNo.(非表示)
        strSQL &= vbCrLf & "     , XMST_PROD_LINE_1F.XPROD_LINE_NAME"                  '生産ﾗｲﾝﾏｽﾀ(1F)            .生産ﾗｲﾝ名称
        strSQL &= vbCrLf & "     , TMST_ITEM.XHINMEI_CD "                               '品目ﾏｽﾀ                    .品名ｺｰﾄﾞ
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.FHINMEI_CD "              '包材出庫設定状態(1F)      .品名記号
        strSQL &= vbCrLf & "     , TMST_ITEM.FHINMEI "                                  '品目ﾏｽﾀ                    .品名
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.XMAKER_CD "               '包材出庫設定状態(1F)      .ﾒｰｶｰｺｰﾄﾞ(非表示)
        strSQL &= vbCrLf & "     , XMST_WRAPPING_MAKER.XMAKER_NAME "                    '包材ﾒｰｶｰﾏｽﾀ                .ﾒｰｶ名

        strSQL &= vbCrLf & "     , TMST_TRK.FBUF_NAME "
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.XOUT_PALLET_NUM "         '包材出庫設定状態(1F)      .出庫PL設定枚数
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.XPLAN_NUM "               '包材出庫設定状態(1F)      .要求PL数
        '在庫PL数
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.XRESULT_NUM_SUM "         '包材出庫設定状態(1F)      .出庫済PL数
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.XSTART_DT "               '包材出庫設定状態(1F)      .登録日時
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.FEQ_ID "                  '包材出庫設定状態(1F)      .設備ID(非表示)
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.FTRK_BUF_NO "             '包材出庫設定状態(1F)      .ﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｧNo.(非表示)
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.XPROGRESS "               '包材出庫設定状態(1F)      .進捗状態(非表示)
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.FGRID_DISPLAYINDEX "      '包材出庫設定状態(1F)      .ｸﾞﾘｯﾄﾞ列表示順序(非表示)

        '============================================================
        'FROM
        '============================================================
        strSQL &= vbCrLf & " FROM "
        strSQL &= vbCrLf & "    XSTS_WRAPPING_MATERIAL_1F "    '【包材出庫設定状態(1F)】
        strSQL &= vbCrLf & "  , XMST_PROD_LINE_1F "            '【生産ﾗｲﾝﾏｽﾀ(1F)】
        ...
        strSQL &= vbCrLf & "    AND XSTS_WRAPPING_MATERIAL_1F.FHINMEI_CD = TMST_ITEM.FHINMEI_CD(+) "
        strSQL &= vbCrLf & "    AND XSTS_WRAPPING_MATERIAL_1F.XPROD_LINE = XMST_PROD_LINE_1F.XPROD_LINE(+) "
        strSQL &= vbCrLf & "    AND XSTS_WRAPPING_MATERIAL_1F.XMAKER_CD = XMST_WRAPPING_MAKER.XMAKER_CD(+) "
        strSQL &= vbCrLf & "    AND XSTS_WRAPPING_MATERIAL_1F.FTRK_BUF_NO = TMST_TRK.FTRK_BUF_NO(+) "

        '2017/07/06 ADD INFOMATE
        strSQL &= vbCrLf & "    AND XMST_PROD_LINE_1F.FDISP_VALUE=" & cboFBUF_NAME.SelectedIndex.ToString
        ...
        strSQL &= vbCrLf & "    XSTS_WRAPPING_MATERIAL_1F.FGRID_DISPLAYINDEX"          '包材出庫設定状態(1F)      .ｸﾞﾘｯﾄﾞ列表示順序
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            For Each objRow In objDataSet.Tables(strDataSetName).Rows
                ...
                ' ﾃﾞｰﾀ取得
                '********************************************************************
                Dim XPROD_LINE As String = ""                   '包材出庫設定状態(1F) .生産ﾗｲﾝNo.
                Dim XPROD_LINE_NAME As String = ""              '生産ﾗｲﾝﾏｽﾀ(1F)       .生産ﾗｲﾝ名称
                Dim XHINMEI_CD As String = ""                   '品目ﾏｽﾀ               .品名ｺｰﾄﾞ
                Dim FHINMEI_CD As String = ""                   '包材出庫設定状態(1F) .品名記号
                Dim FHINMEI As String = ""                      '品目ﾏｽﾀ               .品名
                Dim XMAKER_CD As String = ""                    '包材出庫設定状態(1F) .ﾒｰｶｰｺｰﾄﾞ
                Dim XMAKER_NAME As String = ""                  '包材ﾒｰｶｰﾏｽﾀ           .ﾒｰｶ名
                Dim XOUT_PALLET_NUM As String = ""              '包材出庫設定状態(1F) .出庫PL設定枚数
                Dim YOUKYUU_NUM As String = ""                  '包材出庫設定状態(1F) .要求PL数
                'Dim ZAIKO_NUM As String = ""                    '包材出庫設定状態(1F) .在庫PL数
                Dim XRESULT_NUM As String = ""                  '包材出庫設定状態(1F) .出庫済PL数
                Dim XSTART_DT As String = ""                    '包材出庫設定状態(1F) .登録日時
                Dim FEQ_ID As String = ""                       '包材出庫設定状態(1F) .設備ID(非表示)
                Dim FTRK_BUF_NO As String = ""                  '包材出庫設定状態(1F) .ﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｧNo.(非表示)
                Dim XPROGRESS As String = ""                    '包材出庫設定状態(1F) .進捗状態(非表示)
                Dim FGRID_DISPLAYINDEX As String = ""           '包材出庫設定状態(1F) .ｸﾞﾘｯﾄﾞ列表示順序(非表示)
                ...
                XOUT_PALLET_NUM = TO_STRING(objRow("XOUT_PALLET_NUM"))
                YOUKYUU_NUM = TO_STRING(objRow("XPLAN_NUM"))
                ST_NAME = TO_STRING(objRow("FBUF_NAME"))
...
#End Region
    ...
    'Private Sub FRM_310050_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

    'End Sub

    Private Sub cboFBUF_NAME_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboFBUF_NAME.SelectedIndexChanged
        '*********************************************
        ' ｸﾞﾘｯﾄﾞ表示
        '*********************************************
        Call grdListDisplaySub(grdList)

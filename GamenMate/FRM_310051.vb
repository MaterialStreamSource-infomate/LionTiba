...
#Region "  共通変数　                               "
    ...
    Private mstrOUT_ST As String                     '出庫ST
    Private mstrOUT_ST_NAME As String                     '出庫ST
    Private mstrXOUT_PALLET_NUM As String
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義　                            "
    ...
    ''' =======================================
    ''' <summary>コンボボックスインデックス</summary>
    ''' <remarks></remarks>
    ''' =======================================
    Public Property intCboIndex() As String
        Get
            Return mintCboIndex
        End Get
        Set(ByVal value As String)
...
#Region "  ﾌｫｰﾑﾛｰﾄﾞ     処理                        "
    ...
    Private Sub LoadProcess()
        ...
        strSQL &= vbCrLf & "     , XMST_PROD_LINE_1F.XPROD_LINE_NAME "                 '生産ﾗｲﾝﾏｽﾀ(1F)            .生産ﾗｲﾝ名
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.FHINMEI_CD "              '包材出庫設定状態(1F)      .品名記号
        strSQL &= vbCrLf & "     , TMST_ITEM.FHINMEI "                                  '品目ﾏｽﾀ                    .品名
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.XMAKER_CD "               '包材出庫設定状態(1F)      .ﾒｰｶｰｺｰﾄﾞ
        ...
        strSQL &= vbCrLf & "     , XSTS_WRAPPING_MATERIAL_1F.FHINMEI_CD DEFAULT_FHINMEI_CD "   '包材出庫設定状態(1F)            .品名記号
        ...
        strSQL &= vbCrLf & "    AND XSTS_WRAPPING_MATERIAL_1F.XPROD_LINE = '" & mstrXPROD_LINE & "' "
        ...
        gobjDb.SQL = strSQL
        objDataSet.Clear()
        strDataSetName = "XSTS_WRAPPING_MATERIAL_1F"
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            For Each objRow In objDataSet.Tables(strDataSetName).Rows
                ...
                mstrOUT_ST = TO_STRING(objRow("FTRK_BUF_NO"))
                mstrOUT_ST_NAME = TO_STRING(objRow("FBUF_NAME"))
                mstrXOUT_PALLET_NUM = TO_STRING(objRow("XOUT_PALLET_NUM"))
        ...
        If mintFPROGRESS = XPROGRESS_NON Then
            ...
            '出庫STｺﾝﾎﾞﾎﾞｯｸｽ  ｾｯﾄ
            '===================================
            Call cboOUT_ST_Setup(cmbOUT_ST)
        ...
        ElseIf mintFPROGRESS = XPROGRESS_START Then
            ...
            Call cboOUT_ST_Setup(cmbOUT_ST)
            If mstrOUT_ST <> "" Then
                cmbOUT_ST.Text = mstrOUT_ST_NAME
            End If
            cmbOUT_ST.Enabled = False
            ...
            txtXOUT_PALLET_NUM.Text = mstrXOUT_PALLET_NUM
            txtXOUT_PALLET_NUM.Enabled = False
...
#Region "  ｿｹｯﾄ送信(出庫登録)                   "
    ...
    Private Function SendSocket01() As Boolean
        ...
        objTelegram.FORMAT_ID = FORMAT_DSP_PRI & FSYORI_ID_J400205    'ﾌｫｰﾏｯﾄ名ｾｯﾄ

        objTelegram.SETIN_DATA("XDSPPROD_LINE", mstrXPROD_LINE)                             '生産ﾗｲﾝNo. 
        objTelegram.SETIN_DATA("DSPTRK_BUF_NO", cmbOUT_ST.SelectedValue)                            'ﾊﾞｯﾌｧNo.
        ...
        objTelegram.SETIN_DATA("XDSPPALLET_OUT_NUM", TO_STRING(txtXOUT_PALLET_NUM.Text))   '出庫PL設定枚数
...
#Region "　ｺﾝﾎﾞﾎﾞｯｸｽｾｯﾄ (出庫ST)  "
    ...
    ''' <remarks></remarks>
    ''' *******************************************************************************************************************
    Public Sub cboOUT_ST_Setup(ByRef cboControl As Windows.Forms.ComboBox _
                   , Optional ByVal blnAllFlag As Boolean = True _
                   , Optional ByVal objDefault As Object = Nothing _
                   , Optional ByVal strAllString As String = CBO_ALL_CONTENT_01 _
                     )
        ...
        strSQL = ""
        strSQL &= vbCrLf & " SELECT"
        strSQL &= vbCrLf & " TMST_ST.FTRK_BUF_NO,"
        strSQL &= vbCrLf & " TMST_TRK.FBUF_NAME"
        strSQL &= vbCrLf & " FROM TMST_ST"
        strSQL &= vbCrLf & " JOIN TMST_TRK"
        strSQL &= vbCrLf & " ON TMST_ST.FTRK_BUF_NO=TMST_TRK.FTRK_BUF_NO"

        '↓↓↓↓↓↓************************************************************************************************************
        'JobMate:IKEDA 2017/07/06 生産入庫登録コンボボックス対応 ↓↓↓↓↓↓
        If mintCboIndex = 0 Then
            strSQL &= vbCrLf & " WHERE TMST_ST.FTRK_BUF_NO IN (1171,1172,1173,1174)"
        ElseIf mintCboIndex = 2 Then
            strSQL &= vbCrLf & " WHERE TMST_ST.FTRK_BUF_NO IN (2905,2906,2907,2908,2909)"
        ElseIf mintCboIndex = 1 Then
            strSQL &= vbCrLf & " WHERE TMST_ST.FTRK_BUF_NO IN (2037)"
        End If
        'JobMate:IKEDA 2017/07/06 生産入庫登録コンボボックス対応 ↑↑↑↑↑↑
        '↑↑↑↑↑↑************************************************************************************************************
        ...
        gobjDb.SQL = strSQL
        objDataSet.Clear()
        strDataSetName = "XMST_WRAPPING_ST"
        gobjDb.GetDataSet(strDataSetName, objDataSet)

        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            For Each objRow In objDataSet.Tables(strDataSetName).Rows
                'ｱｲﾃﾑｾｯﾄ
                cboControl.Items.Add(TO_STRING(objRow("FBUF_NAME")))

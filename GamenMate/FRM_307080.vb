...
#Region "　ﾀｲﾏｰ ﾀｲﾑｱｯﾌﾟ                                 "
    ...
    ''' <remarks></remarks>
    ''' *******************************************************************************************************************
    Private Sub tmr307080_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tmr307080.Tick
        Try

            tmr307080.Enabled = False
...
#Region "　ﾌｫｰﾑﾛｰﾄﾞ                                 "
    ...
    Public Overrides Sub InitializeChild()
        ...
        ' 構造体ｾｯﾄ
        '*********************************************
        Call SearchItem_Set(XMAINTE_KUBUN_CV_1F_IN)
        ...
        tmr307080.Interval = TO_INTEGER(gobjComFuncFRM.GetSYS_HEN(FHENSU_ID_JGS307080_001))
        tmr307080.Enabled = True
...
#Region "  F3(2F入庫)  ﾎﾞﾀﾝ押下処理　               "
    ...
    Public Overrides Sub F3Process()
        ...
        ' 構造体ｾｯﾄ
        '*********************************************
        Call SearchItem_Set(XMAINTE_KUBUN_CV_2F_IN)
...
#Region "  F4(1F出庫)  ﾎﾞﾀﾝ押下処理　               "
    ...
    Public Overrides Sub F4Process()
        ...
        ' 構造体ｾｯﾄ
        '*********************************************
        Call SearchItem_Set(XMAINTE_KUBUN_CV_1F_OUT)
...
#Region "  F5(2F出庫)  ﾎﾞﾀﾝ押下処理　               "
    ...
    Public Overrides Sub F5Process()
        ...
        ' 構造体ｾｯﾄ
        '*********************************************
        Call SearchItem_Set(XMAINTE_KUBUN_CV_2F_OUT)
...
#Region "  F6(ﾒﾝﾃﾅﾝｽ)      ﾎﾞﾀﾝ押下処理　           "
    ...
    Public Overrides Sub F6Process()
        ...
        ' ﾌﾟﾛﾊﾟﾃｨｾｯﾄ
        '************************************
        If IsNull(gobjFRM_307081) = False Then
            gobjFRM_307081.Close()
            gobjFRM_307081.Dispose()
            gobjFRM_307081 = Nothing
        End If

        gobjFRM_307081 = New FRM_307081

        Call SetProperty()      'ﾌﾟﾛﾊﾟﾃｨｾｯﾄ

        Dim intRet As DialogResult
        intRet = gobjFRM_307081.ShowDialog()            '展開
...
#Region "  ﾒﾝﾃﾅﾝｽ入力画面へﾌﾟﾛﾊﾟﾃｨｾｯﾄ　             "
    ...
    Private Sub SetProperty()
        ...
        ' 親ﾌﾟﾛﾊﾟﾃｨ
        '********************************************************************
        gobjFRM_307081.userPLC_STNO = TO_STRING(grdList.Item(menmListCol.PLC_STNO, intOyaIndex).Value)                    'ST№
        gobjFRM_307081.userPLC_MODE_VOL1 = TO_STRING(grdList.Item(menmListCol.PLC_MODE_VOL1, intOyaIndex).Value)          '入庫ﾓｰﾄﾞ
        gobjFRM_307081.userPLC_MODE_VOL2 = TO_STRING(grdList.Item(menmListCol.PLC_MODE_VOL2, intOyaIndex).Value)          '出庫ﾓｰﾄﾞ
        gobjFRM_307081.userPLC_PARE_VOL = TO_STRING(grdList.Item(menmListCol.PLC_PARE_VOL, intOyaIndex).Value)            'ﾍﾟｱ
        gobjFRM_307081.userPLC_FORK_VOL1 = TO_STRING(grdList.Item(menmListCol.PLC_FORK_VOL1, intOyaIndex).Value)          'ﾌｫｰｸ2
        gobjFRM_307081.userPLC_FORK_VOL2 = TO_STRING(grdList.Item(menmListCol.PLC_FORK_VOL2, intOyaIndex).Value)          'ﾌｫｰｸ1
        gobjFRM_307081.userPLC_LS = TO_STRING(grdList.Item(menmListCol.PLC_LS, intOyaIndex).Value)                        'L/S
        gobjFRM_307081.userPLC_W_VOL = TO_STRING(grdList.Item(menmListCol.PLC_W_VOL, intOyaIndex).Value)                  'Wﾘｰﾁ
        gobjFRM_307081.userPLC_RETU_VOL = TO_STRING(grdList.Item(menmListCol.PLC_RETU_VOL, intOyaIndex).Value)            '列
        gobjFRM_307081.userPLC_GOUKI = TO_STRING(grdList.Item(menmListCol.PLC_GOUKI, intOyaIndex).Value)                  '号機
        gobjFRM_307081.userPLC_REN = TO_STRING(grdList.Item(menmListCol.PLC_REN, intOyaIndex).Value)                      '連
        gobjFRM_307081.userPLC_END = TO_STRING(grdList.Item(menmListCol.PLC_END, intOyaIndex).Value)                      'END
        gobjFRM_307081.userPLC_RETRY = TO_STRING(grdList.Item(menmListCol.PLC_RETRY, intOyaIndex).Value)                  '入庫再設定
        gobjFRM_307081.userPLC_DAN = TO_STRING(grdList.Item(menmListCol.PLC_DAN, intOyaIndex).Value)                      '段
        gobjFRM_307081.userPLC_GOTO_VOL = TO_STRING(grdList.Item(menmListCol.PLC_GOTO_VOL, intOyaIndex).Value)            '行先
        gobjFRM_307081.userPLC_KO_VOL = TO_STRING(grdList.Item(menmListCol.PLC_KO_VOL, intOyaIndex).Value)                '子
        gobjFRM_307081.userPLC_OYA_VOL = TO_STRING(grdList.Item(menmListCol.PLC_OYA_VOL, intOyaIndex).Value)              '親
        gobjFRM_307081.userPLC_ZAISEKI_VOL = TO_STRING(grdList.Item(menmListCol.PLC_ZAISEKI_VOL, intOyaIndex).Value)      '在席

        '********************************************************************
        ' 子ﾌﾟﾛﾊﾟﾃｨ
        '********************************************************************
        gobjFRM_307081.userPLC_STNO_KO = TO_STRING(grdList.Item(menmListCol.PLC_STNO, intKoIndex).Value)                  'ST№(子)
        gobjFRM_307081.userPLC_MODE_VOL1_KO = TO_STRING(grdList.Item(menmListCol.PLC_MODE_VOL1, intKoIndex).Value)        '入庫ﾓｰﾄﾞ(子)
        gobjFRM_307081.userPLC_MODE_VOL2_KO = TO_STRING(grdList.Item(menmListCol.PLC_MODE_VOL2, intKoIndex).Value)        '出庫ﾓｰﾄﾞ(子)
        gobjFRM_307081.userPLC_PARE_VOL_KO = TO_STRING(grdList.Item(menmListCol.PLC_PARE_VOL, intKoIndex).Value)          'ﾍﾟｱ(子)
        gobjFRM_307081.userPLC_FORK_VOL1_KO = TO_STRING(grdList.Item(menmListCol.PLC_FORK_VOL1, intKoIndex).Value)        'ﾌｫｰｸ2(子)
        gobjFRM_307081.userPLC_FORK_VOL2_KO = TO_STRING(grdList.Item(menmListCol.PLC_FORK_VOL2, intKoIndex).Value)        'ﾌｫｰｸ1(子)
        gobjFRM_307081.userPLC_LS_KO = TO_STRING(grdList.Item(menmListCol.PLC_LS, intKoIndex).Value)                      'L/S(子)
        gobjFRM_307081.userPLC_W_VOL_KO = TO_STRING(grdList.Item(menmListCol.PLC_W_VOL, intKoIndex).Value)                'Wﾘｰﾁ(子)
        gobjFRM_307081.userPLC_RETU_VOL_KO = TO_STRING(grdList.Item(menmListCol.PLC_RETU_VOL, intKoIndex).Value)          '列(子)
        gobjFRM_307081.userPLC_GOUKI_KO = TO_STRING(grdList.Item(menmListCol.PLC_GOUKI, intKoIndex).Value)                '号機(子)
        gobjFRM_307081.userPLC_REN_KO = TO_STRING(grdList.Item(menmListCol.PLC_REN, intKoIndex).Value)                    '連(子)
        gobjFRM_307081.userPLC_END_KO = TO_STRING(grdList.Item(menmListCol.PLC_END, intKoIndex).Value)                    'END(子)
        gobjFRM_307081.userPLC_RETRY_KO = TO_STRING(grdList.Item(menmListCol.PLC_RETRY, intKoIndex).Value)                '入庫再設定(子)
        gobjFRM_307081.userPLC_DAN_KO = TO_STRING(grdList.Item(menmListCol.PLC_DAN, intKoIndex).Value)                    '段(子)
        gobjFRM_307081.userPLC_GOTO_VOL_KO = TO_STRING(grdList.Item(menmListCol.PLC_GOTO_VOL, intKoIndex).Value)          '行先(子)
        gobjFRM_307081.userPLC_KO_VOL_KO = TO_STRING(grdList.Item(menmListCol.PLC_KO_VOL, intKoIndex).Value)              '子(子)
        gobjFRM_307081.userPLC_OYA_VOL_KO = TO_STRING(grdList.Item(menmListCol.PLC_OYA_VOL, intKoIndex).Value)            '親(子)
        gobjFRM_307081.userPLC_ZAISEKI_VOL_KO = TO_STRING(grdList.Item(menmListCol.PLC_ZAISEKI_VOL, intKoIndex).Value)    '在席(子)


        gobjFRM_307081.userXMAINTE_KUBUN = TO_STRING(mudtSEARCH_ITEM.XMAINTE_KUBUN)         'ﾒﾝﾃﾅﾝｽ区分

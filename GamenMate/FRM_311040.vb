...
#End Region
#Region "  ｲﾍﾞﾝﾄ                                    "
#Region "　ｸﾞﾘｯﾄﾞ ﾊﾞｰｽ              選択変更ｲﾍﾞﾝﾄ　 "
    ''' *******************************************************************************************************************
    ''' <summary>
    ''' ｸﾞﾘｯﾄﾞ ﾊﾞｰｽ 選択変更ｲﾍﾞﾝﾄ
    ''' </summary>
    ...
    End Sub
#End Region
#Region "　ｸﾞﾘｯﾄﾞ ﾊﾟﾚｯﾄ積み待ち車輌 選択変更ｲﾍﾞﾝﾄ　 "
    ''' *******************************************************************************************************************
    ''' <summary>
    ''' ｸﾞﾘｯﾄﾞ ﾊﾟﾚｯﾄ積み待ち車輌 選択変更ｲﾍﾞﾝﾄ
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    ''' *******************************************************************************************************************
    Private Sub grdList_PL_Wait_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdList_PL_Wait.SelectionChanged
        Try
            If mFlag_Form_Load = False Then
                ...
                '****************************************
                Call grdListChangeColor_Selection(grdList_PL_Wait)
    ...
    End Sub
#End Region
#Region "　ｸﾞﾘｯﾄﾞ ﾊﾞﾗ積み待ち車輌   選択変更ｲﾍﾞﾝﾄ　 "
    ''' *******************************************************************************************************************
    ''' <summary>
    ''' ｸﾞﾘｯﾄﾞ ﾊﾞﾗ積み待ち車輌 選択変更ｲﾍﾞﾝﾄ
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    ''' *******************************************************************************************************************
    Private Sub grdList_Bara_Wait_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdList_Bara_Wait.SelectionChanged
        Try
            If mFlag_Form_Load = False Then

                '****************************************
                ' ｸﾞﾘｯﾄﾞ ﾊﾞﾗ積み待ち車輌 選択変更処理
                '****************************************
                Call grdListChangeColor_Selection(grdList_Bara_Wait)
...
#Region "　ﾀｲﾏｰ 表示更新                            "
    ...
    ''' <remarks></remarks>
    ''' *******************************************************************************************************************
    Private Sub tmr311040_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tmr311040.Tick
        Try

            tmr311040.Enabled = False


            '**************************************************
            ' 表示更新ﾀｲﾏｰ処理
            '**************************************************
            Call tmr311040_TickProcess()


        Catch ex As Exception
            ComError(ex)
        Finally
            tmr311040.Enabled = True
...
#Region "　ﾌｫｰﾑﾛｰﾄﾞ                                 "
    ...
    Public Overrides Sub InitializeChild()
        ...
        grdListDisplay_PL_Wait()
        grdListDisplay_Bara_Wait()

        Call grdList_Berth.ClearSelection()
        Call grdList_PL_Wait.ClearSelection()
        Call grdList_Bara_Wait.ClearSelection()

        tmr311040.Interval = TO_INTEGER(gobjComFuncFRM.GetSYS_HEN(FHENSU_ID_JGS311040_001))
        tmr311040.Enabled = True
...
#Region "　ﾌｫｰﾑｱﾝﾛｰﾄﾞ                               "
    ...
    Public Overrides Sub CloseChild()
        ...
        grdList_PL_Wait.Dispose()
        grdList_Bara_Wait.Dispose()
    ...
    Public Function InputCheck(ByVal udtCheck_Case As menmCheckCase) As Boolean
        ...
        Select Case udtCheck_Case
            ...
            Case menmCheckCase.YuusenBtn_Click
                ...
                'ｴﾘｱ選択ﾁｪｯｸ
                '--------------
                If grdList_PL_Wait.SelectedRows.Count < 1 And _
                   grdList_Bara_Wait.SelectedRows.Count < 1 Then
...
#Region "　ｸﾞﾘｯﾄﾞ表示(ﾊﾞｰｽ)　                       "
    ...
    Private Sub grdListDisplay_Berth()
        ...
        'Call gobjComFuncFRM.GridSelect(grdList_Berth, intSelectRow, intSelectCol, objPoint)      'ｸﾞﾘｯﾄﾞ選択処理
...
#Region "  優先設定受付         （ID : 400104）     "
    ...
    Private Sub SendSock_400104()
        ...
        If grdList_PL_Wait.SelectedRows.Count > 0 Then
            '(ﾊﾟﾚｯﾄ積み待ち)
            strSYUKKA_D = TO_STRING(grdList_PL_Wait.Item(menmWaitListCol.XSYUKKA_D, grdList_PL_Wait.SelectedRows(0).Index).Value)
            strHENSEI_NO_OYA = TO_STRING(grdList_PL_Wait.Item(menmWaitListCol.XHENSEI_NO_OYA, grdList_PL_Wait.SelectedRows(0).Index).Value)

        ElseIf grdList_Bara_Wait.SelectedRows.Count > 0 Then
            '(ﾊﾞﾗ積み待ち)
            strSYUKKA_D = TO_STRING(grdList_Bara_Wait.Item(menmWaitListCol.XSYUKKA_D, grdList_Bara_Wait.SelectedRows(0).Index).Value)
            strHENSEI_NO_OYA = TO_STRING(grdList_Bara_Wait.Item(menmWaitListCol.XHENSEI_NO_OYA, grdList_Bara_Wait.SelectedRows(0).Index).Value)
...
#Region "  選択変更時のｸﾞﾘｯﾄﾞ色変え                 "
    ...
    Private Sub grdListChangeColor_Selection(ByRef grdList As GamenCommon.cmpMDataGrid)
        ...
        ElseIf grdList Is grdList_PL_Wait Then
            Call grdList_Berth.ClearSelection()
            Call grdList_Bara_Wait.ClearSelection()

        ElseIf grdList Is grdList_Bara_Wait Then
            Call grdList_Berth.ClearSelection()
            Call grdList_PL_Wait.ClearSelection()
...
#Region "  表示更新ﾀｲﾏｰ処理                         "
    ...
    ''' <remarks></remarks>
    ''' *******************************************************************************************************************
    Private Sub tmr311040_TickProcess()
        ...
        ElseIf grdList_PL_Wait.SelectedRows.Count > 0 Then
            '(PL待ち)
            objGrid = grdList_PL_Wait

        ElseIf grdList_Bara_Wait.SelectedRows.Count > 0 Then
            '(ﾊﾞﾗ待ち)
            objGrid = grdList_Bara_Wait
        ...
        grdList_PL_Wait.ScrollBars = ScrollBars.None
        grdList_Bara_Wait.ScrollBars = ScrollBars.None

        Call grdListDisplay_Berth()         'ﾊﾞｰｽ   状態
        Call grdListDisplay_PL_Wait()       'ﾊﾟﾚｯﾄ  積み待ち車輌
        Call grdListDisplay_Bara_Wait()     'ﾊﾞﾗ    積み待ち車輌
        ...
        grdList_PL_Wait.ScrollBars = ScrollBars.Both
        grdList_Bara_Wait.ScrollBars = ScrollBars.Both
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｾｯﾄ(出荷中状況詳細画面)           "
    ...
    Private Sub SetProperty()
        ...
        gobjFRM_311050.userXHENSEI_NO_OYA = TO_STRING(grdList_Berth.Item(menmBerthListCol.XHENSEI_NO_OYA, grdList_Berth.SelectedRows(0).Index).Value)   '親編成No.

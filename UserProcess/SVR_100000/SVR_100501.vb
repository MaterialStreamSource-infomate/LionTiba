...
#Region "  ｸﾗｽ内部処理用定数定義                                                                "
    ...
    '↓↓↓↓↓↓************************************************************************************************************
    'SystemMate:N.Dounoshita 2012/08/24  搬送TO移動ﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｧ№の更新に対応
    Private mFTR_IDOU As Nullable(Of Integer)           '搬送TO移動ﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｧ№
    '↑↑↑↑↑↑************************************************************************************************************
    '↓↓↓↓↓↓************************************************************************************************************
    'SystemMate:N.Dounoshita 2012/08/27  優先ﾚﾍﾞﾙを追加
...
#Region "  出庫ﾄﾗｯｷﾝｸﾞ定義                                                                      "
    ...
    Public Sub UPDATE_OUT_BUF(ByVal decReqNum As Decimal)
        ...
        Try
            ...
            '↓↓↓↓↓↓************************************************************************************************************
            'SystemMate:N.Dounoshita 2012/08/24  搬送TO移動ﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｧ№の更新に対応
            objTPRG_TRK_BUF.FTR_IDOU = mFTR_IDOU                                '搬送TO移動ﾊﾞｯﾌｧ№
            ...
            intRet = objTMST_SAGYO.GET_TMST_SAGYO(True)
            ...
            '↓↓↓↓↓↓************************************************************************************************************
            'SystemMate:N.Dounoshita 2012/08/27  優先ﾚﾍﾞﾙを追加
            If IsNotNull(mFPRIORITY) Then
                objTPLN_CARRY_QUE.FPRIORITY = mFPRIORITY                            '優先ﾚﾍﾞﾙ
            Else
            ...
            objTSTS_HIKIATE.FPLAN_KEY = mFPLAN_KEY                              '計画ｷｰ
            ...
            '棚ﾌﾞﾛｯｸ状態の更新
            '************************************************
            Call UpdateTPRG_TRK_BUF_Relation_XTANA_BLOCK_STS(objTPRG_TRK_BUF, XTANA_BLOCK_STS_NON)
...
#Region "  出庫ﾄﾗｯｷﾝｸﾞ定義(出荷引当用)                                                              "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub UPDATE_OUT_BUF_Syukka(ByVal decReqNum As Decimal)
        Dim intRet As RetCode                   '戻り値
        Dim strMsg As String                    'ﾒｯｾｰｼﾞ
        Try
            ...
            objTPLN_CARRY_QUE.FPALLET_ID = objTPRG_TRK_BUF.FPALLET_ID               'ﾊﾟﾚｯﾄID
            objTPLN_CARRY_QUE.FCARRYQUE_KUBUN = mFCARRYQUE_KUBUN                    '指示区分
            objTPLN_CARRY_QUE.FCARRYQUE_DIR_STS = FCARRYQUE_DIR_STS_JTAIKI          '搬送指示状況
            objTPLN_CARRY_QUE.FENTRY_DT = Now                                       '登録日時
            objTPLN_CARRY_QUE.FUPDATE_DT = Now                                      '更新日時
            objTPLN_CARRY_QUE.ADD_TPLN_CARRY_QUE_ORDER()                            '登録
            ...
            objTSTS_HIKIATE.FPALLET_ID = objTPRG_TRK_BUF.FPALLET_ID             'ﾊﾟﾚｯﾄID
            objTSTS_HIKIATE.FLOT_NO_STOCK = FLOT_NO_STOCK_SKOTEI                '在庫ﾛｯﾄ№
            objTSTS_HIKIATE.FTR_VOL = decReqNum                                 '搬送管理量
            objTSTS_HIKIATE.FTERM_ID = mFTERM_ID                                '端末ID
            objTSTS_HIKIATE.FUSER_ID = mFUSER_ID                                'ﾕｰｻﾞｰID
            '↓↓↓↓↓↓************************************************************************************************************
            'SystemMate:N.Dounoshita 2012/11/07  指示待ち理由の初期値設定

...
#Region "  ｸﾗｽ内部処理用変数定義                                                                "
    ...
    Private Const DSPIN_KUBUN As Integer = 7            '入庫区分
...
#Region "  追加ﾒﾝﾃﾅﾝｽ                                                                           "
    ...
    Private Sub Mente_ADD(ByVal strDenbunDtl() As String)
        Try
            ...
            '包装材料ﾒｰｶﾏｽﾀ存在ﾁｪｯｸ
            '**************************************
            Dim obj_XMST_PROD_LINE_Before As New TBL_XMST_PROD_LINE(Owner, ObjDb, ObjDbLog)
            obj_XMST_PROD_LINE_Before.XPROD_LINE = strDenbunDtl(XDSPPROD_LINE)                  '生産ﾗｲﾝ№
            intRet = obj_XMST_PROD_LINE_Before.GET_XMST_PROD_LINE(False)
            If intRet <> RetCode.NotFound Then
                '(包装材料ﾒｰｶﾏｽﾀが存在した場合)
                strMsg = FRM_MSG_XPROD_LINE_02 & "[生産ﾗｲﾝ№:" & obj_XMST_PROD_LINE_Before.XPROD_LINE & "]"
            ...
            '包装材料ﾒｰｶﾏｽﾀの追加
            '**************************************
            Dim obj_XMST_PROD_LINE_After As New TBL_XMST_PROD_LINE(Owner, ObjDb, ObjDbLog)
            obj_XMST_PROD_LINE_After.XPROD_LINE = strDenbunDtl(XDSPPROD_LINE)                   '生産ﾗｲﾝ№
            obj_XMST_PROD_LINE_After.XPROD_LINE_NAME = strDenbunDtl(XDSPPROD_LINE_NAME)         '生産ﾗｲﾝ名称
            obj_XMST_PROD_LINE_After.FHINMEI_CD = strDenbunDtl(DSPHINMEI_CD)                    '品名ｺｰﾄﾞ
            obj_XMST_PROD_LINE_After.FIN_KUBUN = strDenbunDtl(DSPIN_KUBUN)                      '入庫区分
            Call obj_XMST_PROD_LINE_After.ADD_XMST_PROD_LINE()                                  '追加
...
#Region "  更新ﾒﾝﾃﾅﾝｽ                                                                           "
    ...
    Private Sub Mente_UPDATE(ByVal strDenbunDtl() As String)
        Try
            ...
            '包装材料ﾒｰｶﾏｽﾀの更新
            '**************************************
            Dim obj_XMST_PROD_LINE As New TBL_XMST_PROD_LINE(Owner, ObjDb, ObjDbLog)
            obj_XMST_PROD_LINE.XPROD_LINE = strDenbunDtl(XDSPPROD_LINE)                         '生産ﾗｲﾝ№
            obj_XMST_PROD_LINE.GET_XMST_PROD_LINE(False)
            obj_XMST_PROD_LINE.XPROD_LINE_NAME = strDenbunDtl(XDSPPROD_LINE_NAME)               '生産ﾗｲﾝ名称
            obj_XMST_PROD_LINE.FHINMEI_CD = strDenbunDtl(DSPHINMEI_CD)                          '品名ｺｰﾄﾞ
            obj_XMST_PROD_LINE.FIN_KUBUN = strDenbunDtl(DSPIN_KUBUN)                            '入庫区分
            Call obj_XMST_PROD_LINE.UPDATE_XMST_PROD_LINE()                                     '更新
...
#Region "  削除ﾒﾝﾃﾅﾝｽ                                                                           "
    ...
    Private Sub Mente_DELETE(ByVal strDenbunDtl() As String)
        Try
            ...
            '包装材料ﾒｰｶﾏｽﾀの削除
            '**************************************
            Call obj_XMST_PROD_LINE.DELETE_XMST_PROD_LINE()                      '削除

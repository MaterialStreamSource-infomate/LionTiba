...
#Region "  ｸﾗｽ内部処理用定数定義                                                                "
    ...
    Private Const XDSPPROGRESS As Integer = 12          '進捗状態
    Private Const DSPGRID_DISPLAYINDEX As Integer = 13  'ｸﾞﾘｯﾄﾞ列表示順序
...
#Region "  ﾒｲﾝ処理                                                                              "
    ...
    Public Overrides Function ExecCmd(ByVal strMSG_RECV As String, ByRef strMSG_SEND As String) As RetCode
        ...
        Try
            ...
            objXSTS_WRAPPING_MATERIAL.FTRK_BUF_NO = strDenbunDtl(DSPTRK_BUF_NO)         'ﾄﾗｯｷﾝｸﾞﾊﾞｯﾌｧ№
            intRet = objXSTS_WRAPPING_MATERIAL.GET_XSTS_WRAPPING_MATERIAL(True)
            If intRet = RetCode.OK Then
                '(見つかったとき)

                objXSTS_WRAPPING_MATERIAL.FEQ_ID = strDenbunDtl(DSPEQ_ID)                                                   '設備ID
                objXSTS_WRAPPING_MATERIAL.XPROD_LINE = strDenbunDtl(XDSPPROD_LINE)                                          '生産ﾗｲﾝ№
                objXSTS_WRAPPING_MATERIAL.FHINMEI_CD = strDenbunDtl(DSPHINMEI_CD)                                           '品名ｺｰﾄﾞ
                objXSTS_WRAPPING_MATERIAL.XMAKER_CD = strDenbunDtl(XDSPMAKER_CD)                                            'ﾒｰｶｺｰﾄﾞ
                objXSTS_WRAPPING_MATERIAL.XSTART_DT = TO_DATE_NULLABLE(strDenbunDtl(XDSPSTART_DT))                          '開始日時
                objXSTS_WRAPPING_MATERIAL.XEND_DT = TO_DATE_NULLABLE(strDenbunDtl(XDSPEND_DT))                              '終了日時
                objXSTS_WRAPPING_MATERIAL.XPLAN_NUM = TO_INTEGER_NULLABLE(strDenbunDtl(XDSPPLAN_NUM))                       '計画数量
                objXSTS_WRAPPING_MATERIAL.XRESULT_NUM = TO_INTEGER_NULLABLE(strDenbunDtl(XDSPRESULT_NUM))                   '実績数量
                objXSTS_WRAPPING_MATERIAL.XPROGRESS = TO_INTEGER_NULLABLE(strDenbunDtl(XDSPPROGRESS))                       '進捗状態
                objXSTS_WRAPPING_MATERIAL.FGRID_DISPLAYINDEX = TO_INTEGER_NULLABLE(strDenbunDtl(DSPGRID_DISPLAYINDEX))      'ｸﾞﾘｯﾄﾞ列表示順序

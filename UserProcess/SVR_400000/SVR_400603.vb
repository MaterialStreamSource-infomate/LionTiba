...
#Region "  ｸﾗｽ内部処理用変数定義                                                                "
    ...
    Private Const DSPXSYASYU_KBN As Integer = 4            '車種区分
    Private Const DSPXSYASYU_NAME As Integer = 5           '車種名称
...
#Region "  追加ﾒﾝﾃﾅﾝｽ                                                                           "
    ...
    Private Sub Mente_ADD(ByVal strDenbunDtl() As String)
        Try
            ...
            '輸送手段ﾏｽﾀ存在ﾁｪｯｸ
            '**************************************
            Dim obj_XMST_SYASYU_Before As New TBL_XMST_SYASYU(Owner, ObjDb, ObjDbLog)
            obj_XMST_SYASYU_Before.XSYASYU_KBN = strDenbunDtl(DSPXSYASYU_KBN)    '車種区分
            intRet = obj_XMST_SYASYU_Before.GET_XMST_SYASYU(False)
            If intRet <> RetCode.NotFound Then
                '(輸送手段ﾏｽﾀが存在した場合)
                strMsg = FRM_MSG_XSYASYU_KBN_MSG_01 & "[車種区分:" & obj_XMST_SYASYU_Before.XSYASYU_KBN & "]"
            ...
            '輸送手段ﾏｽﾀの追加
            '**************************************
            Dim obj_XMST_SYASYU_After As New TBL_XMST_SYASYU(Owner, ObjDb, ObjDbLog)
            obj_XMST_SYASYU_After.XSYASYU_KBN = strDenbunDtl(DSPXSYASYU_KBN)                '車種区分
            obj_XMST_SYASYU_After.XSYASYU_NAME = strDenbunDtl(DSPXSYASYU_NAME)              '車種名称
            obj_XMST_SYASYU_After.FENTRY_DT = Now                                           '登録日時
            Call obj_XMST_SYASYU_After.ADD_XMST_SYASYU()                                    '追加
...
#Region "  更新ﾒﾝﾃﾅﾝｽ                                                                           "
    ...
    Private Sub Mente_UPDATE(ByVal strDenbunDtl() As String)
        Try
            ...
            Dim obj_XMST_SYASYU_After As New TBL_XMST_SYASYU(Owner, ObjDb, ObjDbLog)
            obj_XMST_SYASYU_After.XSYASYU_KBN = strDenbunDtl(DSPXSYASYU_KBN)                                   '車種区分
            obj_XMST_SYASYU_After.GET_XMST_SYASYU(False)
            obj_XMST_SYASYU_After.XSYASYU_NAME = strDenbunDtl(DSPXSYASYU_NAME)                                 '車種名称
            Call obj_XMST_SYASYU_After.UPDATE_XMST_SYASYU()                                                 '更新
...
#Region "  削除ﾒﾝﾃﾅﾝｽ                                                                           "
    ...
    Private Sub Mente_DELETE(ByVal strDenbunDtl() As String)
        Try
            ...
            '輸送手段ﾏｽﾀの削除
            '**************************************
            Call obj_XMST_SYASYU_Before.DELETE_XMST_SYASYU()                      '削除

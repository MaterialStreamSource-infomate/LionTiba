...
''' <summary>
''' 画面ｲﾝﾀｰﾌｪｰｽ一時保管ﾃｰﾌﾞﾙｸﾗｽ
''' </summary>
Public Class TBL_XPRG_PARAM_TBL01
...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_XPRG_PARAM_TBL01()                                  '画面ｲﾝﾀｰﾌｪｰｽ一時保管
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ...
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
    ''' </summary>
    Public ReadOnly Property ARYME() As TBL_XPRG_PARAM_TBL01()
        Get
            Return mobjAryMe
        End Get
    End Property
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XPRG_PARAM_TBL01(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        strSQL.Append(vbCrLf & "    XPRG_PARAM_TBL01")
        ...
        objDataSet.Clear()
        strDataSetName = "XPRG_PARAM_TBL01"
...
#Region "  ﾃﾞｰﾀ取得(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XPRG_PARAM_TBL01_ANY() As RetCode
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            For ii = LBound(mobjAryMe) To UBound(mobjAryMe)
                ...
                mobjAryMe(ii) = New TBL_XPRG_PARAM_TBL01(Owner, objDb, objDbLog)
...
#Region "  ﾃﾞｰﾀ取得(ｶｽﾀﾑ抽出)           "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XPRG_PARAM_TBL01_USER(Optional ByRef objUSER_PARAM As Object(,) = Nothing) As RetCode
...
#Region "  ﾃﾞｰﾀ取得(ｶｳﾝﾄ)               "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XPRG_PARAM_TBL01_COUNT() As Integer
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub UPDATE_XPRG_PARAM_TBL01()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        ...
        Else
            ...
            strSQL.Append(vbCrLf & "    AND FPARA_EDA_NO = :" & UBound(strBindField) - 1 & " --ﾊﾟﾗﾒｰﾀ枝番")
        End If
        If IsNull(FSEND_DATA) = True Then
            strSQL.Append(vbCrLf & "    AND FSEND_DATA IS NULL --送信電文")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            ...
            strSQL.Append(vbCrLf & "    AND FSEND_DATA = :" & UBound(strBindField) - 1 & " --送信電文")
        End If
        If IsNull(FUPDATE_DT) = True Then
            strSQL.Append(vbCrLf & "    AND FUPDATE_DT IS NULL --更新日時")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            ...
            strSQL.Append(vbCrLf & "    AND FUPDATE_DT = :" & UBound(strBindField) - 1 & " --更新日時")
        End If
        If IsNull(FENTRY_DT) = True Then
            strSQL.Append(vbCrLf & "    AND FENTRY_DT IS NULL --登録日時")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub ADD_XPRG_PARAM_TBL01()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾃﾞｰﾀ削除                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_XPRG_PARAM_TBL01()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
        ...
        ElseIf IsNull(mFPARA_EDA_NO) = True Then
            strMsg = ERRMSG_ERR_PROPERTY & "[ﾊﾟﾗﾒｰﾀ枝番]"
            Throw New UserException(strMsg)
        ElseIf IsNull(mFSEND_DATA) = True Then
            strMsg = ERRMSG_ERR_PROPERTY & "[送信電文]"
            Throw New UserException(strMsg)
        ElseIf IsNull(mFUPDATE_DT) = True Then
            strMsg = ERRMSG_ERR_PROPERTY & "[更新日時]"
            Throw New UserException(strMsg)
...
#Region "  ﾃﾞｰﾀ削除(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_XPRG_PARAM_TBL01_ANY()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object

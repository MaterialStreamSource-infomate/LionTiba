...
''' <summary>
''' 生産ﾗｲﾝﾏｽﾀ(D45)ﾃｰﾌﾞﾙｸﾗｽ
''' </summary>
Public Class TBL_XMST_PROD_LINE_D45
...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_XMST_PROD_LINE_D45()                                '生産ﾗｲﾝﾏｽﾀ(D45)
    Private mstrUSER_SQL As String                                               'ﾕｰｻﾞｰSQL
    Private mORDER_BY As String                                                  'OrderBy句
    Private mWHERE As String                                                     'Where句
    Private mXPROD_LINE As String                                                '生産ﾗｲﾝ№
    Private mXPROD_LINE_NAME As String                                           '生産ﾗｲﾝ名称
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ...
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
    ''' </summary>
    Public ReadOnly Property ARYME() As TBL_XMST_PROD_LINE_D45()
        Get
            Return mobjAryMe
        End Get
    End Property
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XMST_PROD_LINE_D45(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        strSQL.Append(vbCrLf & "    XMST_PROD_LINE_D45")
        ...
        If IsNull(XPROD_LINE_NAME) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND XPROD_LINE_NAME = :" & UBound(strBindField) - 1 & " --生産ﾗｲﾝ名称")
        End If
        ...
        objDataSet.Clear()
        strDataSetName = "XMST_PROD_LINE_D45"
...
#Region "  ﾃﾞｰﾀ取得(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XMST_PROD_LINE_D45_ANY() As RetCode
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            For ii = LBound(mobjAryMe) To UBound(mobjAryMe)
                ...
                mobjAryMe(ii) = New TBL_XMST_PROD_LINE_D45(Owner, objDb, objDbLog)
...
#Region "  ﾃﾞｰﾀ取得(ｶｽﾀﾑ抽出)           "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XMST_PROD_LINE_D45_USER(Optional ByRef objUSER_PARAM AS Object(,) = nothing) As RetCode
...
#Region "  ﾃﾞｰﾀ取得(ｶｳﾝﾄ)               "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XMST_PROD_LINE_D45_COUNT() As Integer
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub UPDATE_XMST_PROD_LINE_D45()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        ...
        If IsNull(mXPROD_LINE_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XPROD_LINE_NAME = NULL --生産ﾗｲﾝ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XPROD_LINE_NAME = NULL --生産ﾗｲﾝ名称")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXPROD_LINE_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XPROD_LINE_NAME = :" & Ubound(strBindField) - 1 & " --生産ﾗｲﾝ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XPROD_LINE_NAME = :" & Ubound(strBindField) - 1 & " --生産ﾗｲﾝ名称")
        End If
        intCount = intCount + 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub ADD_XMST_PROD_LINE_D45()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
        ...
        intCount = intCount + 1
        If IsNull(mXPROD_LINE_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --生産ﾗｲﾝ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --生産ﾗｲﾝ名称")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXPROD_LINE_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --生産ﾗｲﾝ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --生産ﾗｲﾝ名称")
        End If
        intCount = intCount + 1
        If IsNull(mFHINMEI_CD) = True Then
...
#Region "  ﾃﾞｰﾀ削除                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_XMST_PROD_LINE_D45()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾃﾞｰﾀ削除(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_XMST_PROD_LINE_D45_ANY()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("XPROD_LINE_NAME")) = False Then mXPROD_LINE_NAME = objObject.XPROD_LINE_NAME                                 '生産ﾗｲﾝ名称

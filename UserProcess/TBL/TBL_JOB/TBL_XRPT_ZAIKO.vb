...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_XRPT_ZAIKO()                                        '在庫日報
    ...
    Private mXSTOCK_KOSOU As Nullable(Of Decimal)                                '在庫数
    Private mXSTOCK_PL As Nullable(Of Integer)                                   '在庫ﾊﾟﾚｯﾄ数
#End Region
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ''' <summary>
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
    ''' </summary>
    Public ReadOnly Property ARYME() As TBL_XRPT_ZAIKO()
        Get
            Return mobjAryMe
        End Get
    End Property
    ...
    ''' 在庫数
    ''' </summary>
    Public Property XSTOCK_KOSOU() As Nullable(Of Decimal)
        Get
            Return mXSTOCK_KOSOU
        End Get
    ...
    ''' 在庫ﾊﾟﾚｯﾄ数
    ''' </summary>
    Public Property XSTOCK_PL() As Nullable(Of Integer)
        Get
            Return mXSTOCK_PL
        End Get
        Set(ByVal Value As Nullable(Of Integer))
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XRPT_ZAIKO(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        strSQL.Append(vbCrLf & "    XRPT_ZAIKO")
        ...
        End If
        If IsNull(XSTOCK_KOSOU) = False Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXSTOCK_KOSOU
            strSQL.Append(vbCrLf & "    AND XSTOCK_KOSOU = :" & UBound(strBindField) - 1 & " --在庫数")
        End If
        If IsNull(XSTOCK_PL) = False Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXSTOCK_PL
            strSQL.Append(vbCrLf & "    AND XSTOCK_PL = :" & UBound(strBindField) - 1 & " --在庫ﾊﾟﾚｯﾄ数")
        End If
...
#Region "  ﾃﾞｰﾀ取得(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XRPT_ZAIKO_ANY() As RetCode
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            For ii = LBound(mobjAryMe) To UBound(mobjAryMe)
                ...
                mobjAryMe(ii) = New TBL_XRPT_ZAIKO(Owner, objDb, objDbLog)
...
#Region "  ﾃﾞｰﾀ取得(ｶｽﾀﾑ抽出)           "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XRPT_ZAIKO_USER(Optional ByRef objUSER_PARAM AS Object(,) = nothing) As RetCode
...
#Region "  ﾃﾞｰﾀ取得(ｶｳﾝﾄ)               "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XRPT_ZAIKO_COUNT() As Integer
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    Public Sub UPDATE_XRPT_ZAIKO()
        ...
        End If
        intCount = intCount + 1
        If IsNull(mXSTOCK_KOSOU) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XSTOCK_KOSOU = NULL --在庫数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XSTOCK_KOSOU = NULL --在庫数")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXSTOCK_KOSOU
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XSTOCK_KOSOU = :" & Ubound(strBindField) - 1 & " --在庫数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XSTOCK_KOSOU = :" & Ubound(strBindField) - 1 & " --在庫数")
        End If
        intCount = intCount + 1
        If IsNull(mXSTOCK_PL) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XSTOCK_PL = NULL --在庫ﾊﾟﾚｯﾄ数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XSTOCK_PL = NULL --在庫ﾊﾟﾚｯﾄ数")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXSTOCK_PL
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XSTOCK_PL = :" & Ubound(strBindField) - 1 & " --在庫ﾊﾟﾚｯﾄ数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XSTOCK_PL = :" & Ubound(strBindField) - 1 & " --在庫ﾊﾟﾚｯﾄ数")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " WHERE")
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_XRPT_ZAIKO()
        ...
        intCount = intCount + 1
        If IsNull(mXSTOCK_KOSOU) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --在庫数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --在庫数")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXSTOCK_KOSOU
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --在庫数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --在庫数")
        End If
        intCount = intCount + 1
        If IsNull(mXSTOCK_PL) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --在庫ﾊﾟﾚｯﾄ数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --在庫ﾊﾟﾚｯﾄ数")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXSTOCK_PL
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --在庫ﾊﾟﾚｯﾄ数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --在庫ﾊﾟﾚｯﾄ数")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " )")
...
#Region "  ﾃﾞｰﾀ削除(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_XRPT_ZAIKO_ANY()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
        ...
        If IsNotNull(FHINMEI_CD) = True Then
            ...
            strSQL.Append(vbCrLf & "    AND FHINMEI_CD = :" & UBound(strBindField) - 1 & " --品目ｺｰﾄﾞ")
        End If
        If IsNotNull(XSTOCK_KOSOU) = True Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXSTOCK_KOSOU
            strSQL.Append(vbCrLf & "    AND XSTOCK_KOSOU = :" & UBound(strBindField) - 1 & " --在庫数")
        End If
        If IsNotNull(XSTOCK_PL) = True Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXSTOCK_PL
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("XSTOCK_KOSOU")) = False Then mXSTOCK_KOSOU = objObject.XSTOCK_KOSOU                                          '在庫数
        If IsNothing(objType.GetProperty("XSTOCK_PL")) = False Then mXSTOCK_PL = objObject.XSTOCK_PL                                                   '在庫ﾊﾟﾚｯﾄ数
...
#Region "  ﾃﾞｰﾀ→変数                   "
    ...
    Private Sub SET_DATA(ByVal objRow As DataRow)
        ...
        mXSTOCK_KOSOU = TO_DECIMAL_NULLABLE(objRow("XSTOCK_KOSOU"))
        mXSTOCK_PL = TO_INTEGER_NULLABLE(objRow("XSTOCK_PL"))
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        End If
        If IsNotNull(XSTOCK_KOSOU) Then
            strMsg &= "[在庫数:" & XSTOCK_KOSOU & "]"
        End If
        If IsNotNull(XSTOCK_PL) Then
            strMsg &= "[在庫ﾊﾟﾚｯﾄ数:" & XSTOCK_PL & "]"
        End If

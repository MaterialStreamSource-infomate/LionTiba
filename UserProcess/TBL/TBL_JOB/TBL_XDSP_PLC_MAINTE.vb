...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_XDSP_PLC_MAINTE()                                   'ﾒﾝﾃﾅﾝｽ表示ﾏｽﾀ
    Private mstrUSER_SQL As String                                               'ﾕｰｻﾞｰSQL
    Private mORDER_BY As String                                                  'OrderBy句
    Private mWHERE As String                                                     'Where句
    Private mXEQ_ID_MAINTE As String                                             '表示設備ID
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    Public Function GET_XDSP_PLC_MAINTE(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        If IsNull(XEQ_ID_MAINTE) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND XEQ_ID_MAINTE = :" & UBound(strBindField) - 1 & " --表示設備ID")
        End If
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    Public Sub UPDATE_XDSP_PLC_MAINTE()
        ...
        If IsNull(mXEQ_ID_MAINTE) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XEQ_ID_MAINTE = NULL --表示設備ID")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XEQ_ID_MAINTE = NULL --表示設備ID")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXEQ_ID_MAINTE
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XEQ_ID_MAINTE = :" & Ubound(strBindField) - 1 & " --表示設備ID")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XEQ_ID_MAINTE = :" & Ubound(strBindField) - 1 & " --表示設備ID")
        End If
        intCount = intCount + 1
        ...
        If IsNull(XEQ_ID_MAINTE) = True Then
            strSQL.Append(vbCrLf & "    AND XEQ_ID_MAINTE IS NULL --表示設備ID")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_XDSP_PLC_MAINTE()
        ...
        Dim intCount As Integer = 0
        If IsNull(mXEQ_ID_MAINTE) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --表示設備ID")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --表示設備ID")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXEQ_ID_MAINTE
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --表示設備ID")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --表示設備ID")
        End If
        intCount = intCount + 1
        If IsNull(mXMAINTE_KUBUN) = True Then
...
#Region "  ﾃﾞｰﾀ削除                     "
    ...
    Public Sub DELETE_XDSP_PLC_MAINTE()
        ...
        ElseIf IsNull(mXEQ_ID_MAINTE) = True Then
            strMsg = ERRMSG_ERR_PROPERTY & "[表示設備ID]"
            Throw New UserException(strMsg)
        End If
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("XEQ_ID_MAINTE")) = False Then mXEQ_ID_MAINTE = objObject.XEQ_ID_MAINTE '表示設備ID
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        strMsg &= "[ﾃｰﾌﾞﾙ名:ﾒﾝﾃﾅﾝｽ表示ﾏｽﾀ]"
        If IsNotNull(XEQ_ID_MAINTE) Then
            strMsg &= "[表示設備ID:" & XEQ_ID_MAINTE & "]"
        End If

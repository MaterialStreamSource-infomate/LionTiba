...
''' <summary>
''' 包材出庫設定状態(1F)ﾃｰﾌﾞﾙｸﾗｽ
''' </summary>
Public Class TBL_XSTS_WRAPPING_MATERIAL_1F
...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_XSTS_WRAPPING_MATERIAL_1F()                         '包材出庫設定状態(1F)
    ...
    Private mXOUT_PALLET_NUM As Nullable(Of Integer)                             '出庫PL設定枚数
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ...
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
    ''' </summary>
    Public ReadOnly Property ARYME() As TBL_XSTS_WRAPPING_MATERIAL_1F()
        Get
            Return mobjAryMe
        End Get
    End Property
    ...
    ''' 出庫PL設定枚数
    ''' </summary>
    Public Property XOUT_PALLET_NUM() As Nullable(Of Integer)
        Get
            Return mXOUT_PALLET_NUM
        End Get
        Set(ByVal Value As Nullable(Of Integer))
            mXOUT_PALLET_NUM = Value
        End Set
    End Property
    ''' <summary>
    ''' 搬送引当量
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XSTS_WRAPPING_MATERIAL_1F(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        strSQL.Append(vbCrLf & "    XSTS_WRAPPING_MATERIAL_1F")
        ...
        End If
        If IsNull(XOUT_PALLET_NUM) = False Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXOUT_PALLET_NUM
            strSQL.Append(vbCrLf & "    AND XOUT_PALLET_NUM = :" & UBound(strBindField) - 1 & " --出庫PL設定枚数")
        End If
...
#Region "  ﾃﾞｰﾀ取得(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XSTS_WRAPPING_MATERIAL_1F_ANY() As RetCode
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            For ii = LBound(mobjAryMe) To UBound(mobjAryMe)
                ...
                mobjAryMe(ii) = New TBL_XSTS_WRAPPING_MATERIAL_1F(Owner, objDb, objDbLog)
...
#Region "  ﾃﾞｰﾀ取得(ｶｽﾀﾑ抽出)           "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XSTS_WRAPPING_MATERIAL_1F_USER(Optional ByRef objUSER_PARAM As Object(,) = Nothing) As RetCode
...
#Region "  ﾃﾞｰﾀ取得(ｶｳﾝﾄ)               "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_XSTS_WRAPPING_MATERIAL_1F_COUNT() As Integer
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub UPDATE_XSTS_WRAPPING_MATERIAL_1F()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        ...
        End If
        intCount = intCount + 1
        If IsNull(mXOUT_PALLET_NUM) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XOUT_PALLET_NUM = NULL --出庫PL設定枚数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XOUT_PALLET_NUM = NULL --出庫PL設定枚数")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXOUT_PALLET_NUM
            If intCount = 0 Then strSQL.Append(vbCrLf & "    XOUT_PALLET_NUM = :" & Ubound(strBindField) - 1 & " --出庫PL設定枚数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,XOUT_PALLET_NUM = :" & Ubound(strBindField) - 1 & " --出庫PL設定枚数")
        End If
        intCount = intCount + 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub ADD_XSTS_WRAPPING_MATERIAL_1F()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
        ...
        intCount = intCount + 1
        If IsNull(mXOUT_PALLET_NUM) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --出庫PL設定枚数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --出庫PL設定枚数")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXOUT_PALLET_NUM
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --出庫PL設定枚数")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --出庫PL設定枚数")
        End If
        intCount = intCount + 1
        If IsNull(mFTR_RES_VOL) = True Then
...
#Region "  ﾃﾞｰﾀ削除                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_XSTS_WRAPPING_MATERIAL_1F()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾃﾞｰﾀ削除(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_XSTS_WRAPPING_MATERIAL_1F_ANY()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
        ...
        If IsNotNull(FPLAN_KEY) = True Then
            ...
            strSQL.Append(vbCrLf & "    AND FPLAN_KEY = :" & UBound(strBindField) - 1 & " --計画ｷｰ")
        End If
        If IsNotNull(XOUT_PALLET_NUM) = True Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mXOUT_PALLET_NUM
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("XOUT_PALLET_NUM")) = False Then mXOUT_PALLET_NUM = objObject.XOUT_PALLET_NUM '出庫PL設定枚数
...
#Region "  ﾃﾞｰﾀ→変数                   "
    ...
    Private Sub SET_DATA(ByVal objRow As DataRow)
        ...
        mXOUT_PALLET_NUM = TO_INTEGER_NULLABLE(objRow("XOUT_PALLET_NUM"))
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        End If
        If IsNotNull(XOUT_PALLET_NUM) Then
            strMsg &= "[出庫PL設定枚数:" & XOUT_PALLET_NUM & "]"
        End If

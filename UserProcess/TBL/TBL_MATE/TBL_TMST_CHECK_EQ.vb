...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TMST_CHECK_EQ()                                     '搬送起動判定ﾏｽﾀ
    ...
    Private mFCHECK_EQ_ID As String                                              '判定設備ID
    Private mFCHECK_EQ_INDEX As Nullable(Of Integer)                             '判定設備ｲﾝﾃﾞｯｸｽ
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    Public Function GET_TMST_CHECK_EQ(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        If IsNull(FCHECK_EQ_ID) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FCHECK_EQ_ID = :" & UBound(strBindField) - 1 & " --判定設備ID")
        End If
        If IsNull(FCHECK_EQ_INDEX) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FCHECK_EQ_INDEX = :" & UBound(strBindField) - 1 & " --判定設備ｲﾝﾃﾞｯｸｽ")
        End If
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    Public Sub UPDATE_TMST_CHECK_EQ()
        ...
        ElseIf IsNull(mFCHECK_EQ_ID) = True Then
            strMsg = ERRMSG_ERR_PROPERTY & "[判定設備ID]"
            Throw New UserException(strMsg)
        End If
        ...
        If IsNull(mFCHECK_EQ_ID) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FCHECK_EQ_ID = NULL --判定設備ID")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FCHECK_EQ_ID = NULL --判定設備ID")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFCHECK_EQ_ID
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FCHECK_EQ_ID = :" & Ubound(strBindField) - 1 & " --判定設備ID")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FCHECK_EQ_ID = :" & Ubound(strBindField) - 1 & " --判定設備ID")
        End If
        intCount = intCount + 1
        If IsNull(mFCHECK_EQ_INDEX) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FCHECK_EQ_INDEX = NULL --判定設備ｲﾝﾃﾞｯｸｽ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FCHECK_EQ_INDEX = NULL --判定設備ｲﾝﾃﾞｯｸｽ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFCHECK_EQ_INDEX
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FCHECK_EQ_INDEX = :" & Ubound(strBindField) - 1 & " --判定設備ｲﾝﾃﾞｯｸｽ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FCHECK_EQ_INDEX = :" & Ubound(strBindField) - 1 & " --判定設備ｲﾝﾃﾞｯｸｽ")
        End If
        intCount = intCount + 1
        ...
        If IsNull(FCHECK_EQ_ID) = True Then
            strSQL.Append(vbCrLf & "    AND FCHECK_EQ_ID IS NULL --判定設備ID")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_TMST_CHECK_EQ()
        ...
        intCount = intCount + 1
        If IsNull(mFCHECK_EQ_ID) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --判定設備ID")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --判定設備ID")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFCHECK_EQ_ID
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --判定設備ID")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --判定設備ID")
        End If
        intCount = intCount + 1
        If IsNull(mFCHECK_EQ_INDEX) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --判定設備ｲﾝﾃﾞｯｸｽ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --判定設備ｲﾝﾃﾞｯｸｽ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFCHECK_EQ_INDEX
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --判定設備ｲﾝﾃﾞｯｸｽ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --判定設備ｲﾝﾃﾞｯｸｽ")
        End If
        intCount = intCount + 1
        If IsNull(mFEQ_STS) = True Then
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FCHECK_EQ_ID")) = False Then mFCHECK_EQ_ID = objObject.FCHECK_EQ_ID '判定設備ID
        If IsNothing(objType.GetProperty("FCHECK_EQ_INDEX")) = False Then mFCHECK_EQ_INDEX = objObject.FCHECK_EQ_INDEX '判定設備ｲﾝﾃﾞｯｸｽ
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        If IsNotNull(FCHECK_EQ_ID) Then
            strMsg &= "[判定設備ID:" & FCHECK_EQ_ID & "]"
        End If
        If IsNotNull(FCHECK_EQ_INDEX) Then
            strMsg &= "[判定設備ｲﾝﾃﾞｯｸｽ:" & FCHECK_EQ_INDEX & "]"
        End If

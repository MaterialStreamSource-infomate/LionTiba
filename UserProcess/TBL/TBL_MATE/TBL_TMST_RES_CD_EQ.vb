...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TMST_RES_CD_EQ()                                    '応答ｺｰﾄﾞ処理ﾏｽﾀ
    ...
    Private mFRES_CD_EQ_NAME As String                                           '設備応答ｺｰﾄﾞ名称
    Private mFMSG_FLAG As Nullable(Of Integer)                                   'ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ
    Private mFCUT_FLAG As Nullable(Of Integer)                                   '設備切離ﾌﾗｸﾞ
#End Region
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ''' <summary>
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    Public Function GET_TMST_RES_CD_EQ(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        If IsNull(FRES_CD_EQ_NAME) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FRES_CD_EQ_NAME = :" & UBound(strBindField) - 1 & " --設備応答ｺｰﾄﾞ名称")
        End If
        If IsNull(FMSG_FLAG) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FMSG_FLAG = :" & UBound(strBindField) - 1 & " --ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ")
        End If
        If IsNull(FCUT_FLAG) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FCUT_FLAG = :" & UBound(strBindField) - 1 & " --設備切離ﾌﾗｸﾞ")
        End If
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    Public Sub UPDATE_TMST_RES_CD_EQ()
        ...
        If IsNull(mFRES_CD_EQ_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FRES_CD_EQ_NAME = NULL --設備応答ｺｰﾄﾞ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FRES_CD_EQ_NAME = NULL --設備応答ｺｰﾄﾞ名称")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFRES_CD_EQ_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FRES_CD_EQ_NAME = :" & Ubound(strBindField) - 1 & " --設備応答ｺｰﾄﾞ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FRES_CD_EQ_NAME = :" & Ubound(strBindField) - 1 & " --設備応答ｺｰﾄﾞ名称")
        End If
        intCount = intCount + 1
        If IsNull(mFMSG_FLAG) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FMSG_FLAG = NULL --ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FMSG_FLAG = NULL --ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFMSG_FLAG
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FMSG_FLAG = :" & Ubound(strBindField) - 1 & " --ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FMSG_FLAG = :" & Ubound(strBindField) - 1 & " --ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ")
        End If
        intCount = intCount + 1
        If IsNull(mFCUT_FLAG) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FCUT_FLAG = NULL --設備切離ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FCUT_FLAG = NULL --設備切離ﾌﾗｸﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFCUT_FLAG
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FCUT_FLAG = :" & Ubound(strBindField) - 1 & " --設備切離ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FCUT_FLAG = :" & Ubound(strBindField) - 1 & " --設備切離ﾌﾗｸﾞ")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " WHERE")
        ...
        If IsNull(FDIRECTION) = True Then
            strSQL.Append(vbCrLf & "    AND FDIRECTION IS NULL --方向")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
        ...
        If IsNull(FRES_CD_EQ) = True Then
            strSQL.Append(vbCrLf & "    AND FRES_CD_EQ IS NULL --設備応答ｺｰﾄﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_TMST_RES_CD_EQ()
        ...
        intCount = intCount + 1
        If IsNull(mFRES_CD_EQ_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --設備応答ｺｰﾄﾞ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --設備応答ｺｰﾄﾞ名称")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFRES_CD_EQ_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --設備応答ｺｰﾄﾞ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --設備応答ｺｰﾄﾞ名称")
        End If
        intCount = intCount + 1
        If IsNull(mFMSG_FLAG) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFMSG_FLAG
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ")
        End If
        intCount = intCount + 1
        If IsNull(mFCUT_FLAG) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --設備切離ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --設備切離ﾌﾗｸﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFCUT_FLAG
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --設備切離ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --設備切離ﾌﾗｸﾞ")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " )")
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FRES_CD_EQ_NAME")) = False Then mFRES_CD_EQ_NAME = objObject.FRES_CD_EQ_NAME '設備応答ｺｰﾄﾞ名称
        If IsNothing(objType.GetProperty("FMSG_FLAG")) = False Then mFMSG_FLAG = objObject.FMSG_FLAG 'ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ
        If IsNothing(objType.GetProperty("FCUT_FLAG")) = False Then mFCUT_FLAG = objObject.FCUT_FLAG '設備切離ﾌﾗｸﾞ
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        If IsNotNull(FRES_CD_EQ_NAME) Then
            strMsg &= "[設備応答ｺｰﾄﾞ名称:" & FRES_CD_EQ_NAME & "]"
        End If
        If IsNotNull(FMSG_FLAG) Then
            strMsg &= "[ﾒｯｾｰｼﾞﾛｸﾞ出力ﾌﾗｸﾞ:" & FMSG_FLAG & "]"
        End If
        If IsNotNull(FCUT_FLAG) Then
            strMsg &= "[設備切離ﾌﾗｸﾞ:" & FCUT_FLAG & "]"
        End If

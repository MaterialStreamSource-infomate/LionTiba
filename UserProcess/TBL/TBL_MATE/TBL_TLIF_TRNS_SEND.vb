...
''' 搬送制御送信IFﾃｰﾌﾞﾙｸﾗｽ
''' </summary>
Public Class TBL_TLIF_TRNS_SEND
...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TLIF_TRNS_SEND()                                    '搬送制御送信IF
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ...
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
    ''' </summary>
    Public ReadOnly Property ARYME() As TBL_TLIF_TRNS_SEND()
        Get
            Return mobjAryMe
        End Get
    End Property
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TLIF_TRNS_SEND(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        strSQL.Append(vbCrLf & "    TLIF_TRNS_SEND")
        ...
        End If
        If IsNull(FOFFLINE_FLAG) = False Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFOFFLINE_FLAG
            strSQL.Append(vbCrLf & "    AND FOFFLINE_FLAG = :" & UBound(strBindField) - 1 & " --ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ")
        End If
...
#Region "  ﾃﾞｰﾀ取得(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TLIF_TRNS_SEND_ANY() As RetCode
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            For ii = LBound(mobjAryMe) To UBound(mobjAryMe)
                ...
                mobjAryMe(ii) = New TBL_TLIF_TRNS_SEND(Owner, objDb, objDbLog)
...
#Region "  ﾃﾞｰﾀ取得(ｶｽﾀﾑ抽出)           "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TLIF_TRNS_SEND_USER(Optional ByRef objUSER_PARAM As Object(,) = Nothing) As RetCode
...
#Region "  ﾃﾞｰﾀ取得(ｶｳﾝﾄ)               "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TLIF_TRNS_SEND_COUNT() As Integer
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub UPDATE_TLIF_TRNS_SEND()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        ...
        End If
        intCount = intCount + 1
        If IsNull(mFOFFLINE_FLAG) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FOFFLINE_FLAG = NULL --ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FOFFLINE_FLAG = NULL --ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFOFFLINE_FLAG
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FOFFLINE_FLAG = :" & Ubound(strBindField) - 1 & " --ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FOFFLINE_FLAG = :" & Ubound(strBindField) - 1 & " --ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ")
        End If
        intCount = intCount + 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_TLIF_TRNS_SEND()
        ...
        intCount = intCount + 1
        If IsNull(mFOFFLINE_FLAG) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFOFFLINE_FLAG
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ")
        End If
        intCount = intCount + 1
        If IsNull(mFENTRY_DT) = True Then
...
#Region "  ﾃﾞｰﾀ削除                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_TLIF_TRNS_SEND()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾃﾞｰﾀ削除(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_TLIF_TRNS_SEND_ANY()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
        ...
        If IsNotNull(FRES_CD_EQ) = True Then
            ...
            strSQL.Append(vbCrLf & "    AND FRES_CD_EQ = :" & UBound(strBindField) - 1 & " --設備応答ｺｰﾄﾞ")
        End If
        If IsNotNull(FOFFLINE_FLAG) = True Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFOFFLINE_FLAG
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FOFFLINE_FLAG")) = False Then mFOFFLINE_FLAG = objObject.FOFFLINE_FLAG 'ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ
...
#Region "  ﾃﾞｰﾀ→変数                   "
    ...
    Private Sub SET_DATA(ByVal objRow As DataRow)
        ...
        mFOFFLINE_FLAG = TO_INTEGER_NULLABLE(objRow("FOFFLINE_FLAG"))
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        End If
        If IsNotNull(FOFFLINE_FLAG) Then
            strMsg &= "[ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ:" & FOFFLINE_FLAG & "]"
        End If
...
#End Region
    ...
    '**********************************************************************************************
    '↓↓↓ｼｽﾃﾑ共通
#Region "  搬送制御送信IF追加   [SEQ№発番]                 (Public  ADD_TLIF_TRNS_SEND_SEQ)"
    Public Sub ADD_TLIF_TRNS_SEND_SEQ()
        Try
            ...
            objTPRG_SEQNO.FSEQ_ID = FSEQ_ID_SSVRENTRY_NO_TRNS_S              'ｼｰｹﾝｽID(搬送制御送信IF)
    ...
    End Sub
#End Region
#Region "  搬送制御送信IF削除   [ﾊﾟﾚｯﾄID]                   (Public  DELETE_TLIF_TRNS_SEND_PALLET)"
    Public Sub DELETE_TLIF_TRNS_SEND_PALLET()
        Try
            Dim strSQL As String        'SQL文
            Dim strMsg As String        'ﾒｯｾｰｼﾞ
            Dim intRetSQL As Integer    'SQL実行戻り値
    ...
    End Sub
#End Region
#Region "  ｺﾏﾝﾄﾞIDで進捗状態変更 通信中→正常終了           (Public  UPDATE_TLIF_TRNS_RECV_FPROGRESS_END)"
    Public Function UPDATE_TLIF_TRNS_SEND_FPROGRESS_END() As RetCode
        Try
            Dim strSQL As String            'SQL文
            Dim strMsg As String            'ﾒｯｾｰｼﾞ
            Dim intRetSQL As Integer    'SQL実行戻り値
            ...
            If IsNull(mFEQ_ID) = True Then
                strMsg = ERRMSG_ERR_PROPERTY & "[設備ID]"
                Throw New UserException(strMsg)
                '' ''ElseIf IsNull(mFCOMMAND_ID) = True Then
                '' ''    strMsg = ERRMSG_ERR_PROPERTY & "[ｺﾏﾝﾄﾞID]"
                '' ''    Throw New UserException(strMsg)
            ElseIf IsNull(mFRES_CD_EQ) = True Then
                strMsg = ERRMSG_ERR_PROPERTY & "[設備応答ｺｰﾄﾞ]"
                Throw New UserException(strMsg)
            ...
            strSQL &= vbCrLf & "    TLIF_TRNS_SEND"
            strSQL &= vbCrLf & " SET"
            strSQL &= vbCrLf & "    FPROGRESS = '" & TO_STRING(FPROGRESS_SEND) & "'"             '進捗状態
            strSQL &= vbCrLf & "   ,FRES_CD_EQ = '" & TO_STRING(mFRES_CD_EQ) & "'"              '設備応答ｺｰﾄﾞ
            strSQL &= vbCrLf & "   ,FUPDATE_DT = TO_DATE('" & Format(mFUPDATE_DT, "yyyy/MM/dd HH:mm:ss") & "','YYYY/MM/DD HH24:MI:SS')"         '更新日時
            strSQL &= vbCrLf & " WHERE"
            strSQL &= vbCrLf & "        FEQ_ID = '" & TO_STRING(mFEQ_ID) & "'"                  '設備ID
            If IsNull(mFCOMMAND_ID) = False Then
                strSQL &= vbCrLf & "    AND FCOMMAND_ID = '" & TO_STRING(mFCOMMAND_ID) & "'"    'ｺﾏﾝﾄﾞID
            End If
            If IsNull(mFTEXT_ID) = False Then
                strSQL &= vbCrLf & "    AND FTEXT_ID = '" & TO_STRING(mFTEXT_ID) & "'"          'ﾃｷｽﾄID
            End If
            strSQL &= vbCrLf & "    AND FPROGRESS = '" & TO_STRING(FPROGRESS_SACT) & "'"         '進捗状態
            strSQL &= vbCrLf
            ...
            If intRetSQL < 1 Then
                '(対象行無し)
                '' ''strSQL = Replace(strSQL, vbCrLf, "")
                '' ''strMsg = ERRMSG_ERR_UPDATE & "(対象行無し)[ﾃｰﾌﾞﾙ:TLIF_TRNS_SEND]" & "【" & strSQL & "】"
                '' ''Call Owner.AddToLog(strMsg)
                '' ''Throw New UserException(strMsg)
            End If
    ...
    End Function
#End Region
#Region "  送信失敗ﾚｺｰﾄﾞ取得(正常終了以外)                  (Public  GET_TLIF_TRNS_SEND_ERROR_RECORD)"
    Public Function GET_TLIF_TRNS_SEND_ERROR_RECORD() As RetCode
        Try
            Dim strSQL As String            'SQL文
            Dim strMsg As String            'ﾒｯｾｰｼﾞ
            Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
            ...
            strSQL &= vbCrLf & " WHERE"
            strSQL &= vbCrLf & "        FEQ_ID = '" & TO_STRING(mFEQ_ID) & "'"              '設備ID
            strSQL &= vbCrLf & "    AND FPROGRESS IN ( " & TO_STRING(FPROGRESS_SERR_MCI)     '進捗状態
            strSQL &= vbCrLf & "                      ," & TO_STRING(FPROGRESS_STIMEOUT)     '進捗状態
            strSQL &= vbCrLf & "                      ) "                                   '進捗状態
            strSQL &= vbCrLf
            ObjDb.SQL = strSQL
            objDataSet.Clear()
    ...
    End Function
#End Region
#Region "  未送信ﾚｺｰﾄﾞ配列取得                              (Public  GET_TLIF_TRNS_SEND_SENDARRAY)"
    Public Function GET_TLIF_TRNS_SEND_SENDARRAY() As RetCode
        Try
            ...
            strSQL &= vbCrLf & " WHERE"
            strSQL &= vbCrLf & "        FEQ_ID = '" & TO_STRING(mFEQ_ID) & "'"                  '設備ID
            strSQL &= vbCrLf & "    AND FPROGRESS = '" & TO_STRING(FPROGRESS_SNON) & "'"     '通信状態 = 未処理
            strSQL &= vbCrLf & " ORDER BY "
            strSQL &= vbCrLf & "    FPRIORITY DESC"     '優先ﾚﾍﾞﾙ   降順
            strSQL &= vbCrLf & "  , FENTRY_NO ASC"      '登録ID     昇順
            strSQL &= vbCrLf
            mstrUSER_SQL = strSQL
            udtRet = GET_TLIF_TRNS_SEND_USER()
    ...
    End Function
#End Region
#Region "  異常応答ｺｰﾄﾞﾚｺｰﾄﾞ取得                            (Public  GET_TLIF_TRNS_SEND_RETERR_RECORD)"
    Public Function GET_TLIF_TRNS_SEND_RETERR_RECORD() As RetCode
        Try
            Dim strSQL As String            'SQL文
            Dim strMsg As String            'ﾒｯｾｰｼﾞ
            Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
            ...
            strSQL &= vbCrLf & " WHERE"
            strSQL &= vbCrLf & "        FEQ_ID = '" & TO_STRING(mFEQ_ID) & "'"          '設備ID
            strSQL &= vbCrLf & "    AND FRES_CD_EQ <> 0 "                               '設備応答ｺｰﾄﾞ
            strSQL &= vbCrLf
            ObjDb.SQL = strSQL
            objDataSet.Clear()
            strDataSetName = "TLIF_TRNS_SEND"
    ...
    End Function
#End Region
#Region "  未送信&送信中ﾚｺｰﾄﾞ件数取得                       (Public  GET_TLIF_TRNS_SEND_COUNT_BY_EQ_ID)"
    Public Function GET_TLIF_TRNS_SEND_COUNT_BY_EQ_ID() As RetCode
        Try
            Dim strSQL As String            'SQL文
            Dim strMsg As String            'ﾒｯｾｰｼﾞ
            Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
    ...
    End Function
#End Region
#Region "  再送信待ち&異常終了ﾚｺｰﾄﾞ件数取得                 (Public  GET_TLIF_TRNS_SEND_COUNT_BY_EQ_ID_TRY)"
    Public Function GET_TLIF_TRNS_SEND_COUNT_BY_EQ_ID_TRY() As RetCode
        Try
            Dim strSQL As String            'SQL文
            Dim strMsg As String            'ﾒｯｾｰｼﾞ
            Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
            ...
            strSQL &= vbCrLf & " WHERE"
            strSQL &= vbCrLf & "        FEQ_ID = '" & TO_STRING(mFEQ_ID) & "'"                  '設備ID
            strSQL &= vbCrLf & "    AND FPROGRESS IN('" & TO_STRING(FPROGRESS_STRY) & "','" & TO_STRING(FPROGRESS_SERR_MCI) & "','" & TO_STRING(FPROGRESS_SERR_SVR) & "')"
            strSQL &= vbCrLf & " ORDER BY "
            strSQL &= vbCrLf & "    FPRIORITY DESC"     '優先ﾚﾍﾞﾙ   降順
            strSQL &= vbCrLf & "  , FENTRY_NO ASC"      '登録ID     昇順
            strSQL &= vbCrLf
    ...
    End Function
#End Region
#Region "  (通信状態 = 通信中)一行取得                      (Public  GET_TLIF_TRNS_SEND_INTCOMM_STS_ACT)"
    Public Function GET_TLIF_TRNS_SEND_INTCOMM_STS_ACT() As RetCode
        Try
            Dim strSQL As String            'SQL文
            Dim strMsg As String            'ﾒｯｾｰｼﾞ
            Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
            ...
            strSQL &= vbCrLf & " WHERE"
            strSQL &= vbCrLf & "        FEQ_ID = '" & TO_STRING(mFEQ_ID) & "'"                      '設備ID
            strSQL &= vbCrLf & "    AND FPROGRESS = '" & TO_STRING(FPROGRESS_SACT) & "'"             '通信状態 = 通信中
            strSQL &= vbCrLf & " ORDER BY "
            strSQL &= vbCrLf & "    FENTRY_NO "
            strSQL &= vbCrLf
    ...
    End Function
#End Region
#Region "  最新一行取得                                     (Public  GET_TLIF_TRNS_SEND_LATEST)"
    Public Function GET_TLIF_TRNS_SEND_LATEST() As RetCode
        Try
            Dim strSQL As String            'SQL文
            Dim strMsg As String            'ﾒｯｾｰｼﾞ
            Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
    ...
    End Function
#End Region
#Region "  ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ=FLAG_ONの最新一行取得              (Public  GET_TLIF_TRNS_SEND_LATEST_FOFFLINE_FLAG)"
    Public Function GET_TLIF_TRNS_SEND_LATEST_FOFFLINE_FLAG() As RetCode
        Try
            Dim strSQL As String            'SQL文
            Dim strMsg As String            'ﾒｯｾｰｼﾞ
            Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
            ...
            strSQL &= vbCrLf & " WHERE"
            strSQL &= vbCrLf & "        FEQ_ID = '" & TO_STRING(mFEQ_ID) & "'"                      '設備ID
            strSQL &= vbCrLf & "    AND FPROGRESS = '" & TO_STRING(FPROGRESS_SNON) & "'"            '進捗状態 = 未実施
            strSQL &= vbCrLf & "    AND FOFFLINE_FLAG = " & FLAG_ON                                 'ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ = FLAG_ON
            strSQL &= vbCrLf & " ORDER BY "
            strSQL &= vbCrLf & "    FPRIORITY DESC"     '優先ﾚﾍﾞﾙ   降順
            strSQL &= vbCrLf & "  , FENTRY_NO ASC"      '登録ID     昇順
            strSQL &= vbCrLf
...
#End Region
    ...
    '**********************************************************************************************
    '↓↓↓ｼｽﾃﾑ固有
#Region "  最新全行取得                                     (Public  GET_TLIF_TRNS_SEND_LATEST_ANY)"
    Public Function GET_TLIF_TRNS_SEND_LATEST_ANY() As RetCode
        Try
            Dim strSQL As String                'SQL文
            Dim strMsg As String                'ﾒｯｾｰｼﾞ
            Dim intRet As RetCode = RetCode.NG  '戻り値
            ...
            '***********************
            mstrUSER_SQL = strSQL
            intRet = GET_TLIF_TRNS_SEND_USER()
    ...
    End Function
#End Region
#Region "  ｵﾌﾗｲﾝ送信ﾌﾗｸﾞ=FLAG_ONの最新全行取得              (Public  GET_TLIF_TRNS_SEND_LATEST_FOFFLINE_FLAG_ANY)"
    Public Function GET_TLIF_TRNS_SEND_LATEST_FOFFLINE_FLAG_ANY() As RetCode
        Try
            Dim strSQL As String                'SQL文
            Dim strMsg As String                'ﾒｯｾｰｼﾞ
            Dim intRet As RetCode = RetCode.NG  '戻り値

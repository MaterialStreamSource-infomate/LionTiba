...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TDSP_TERM()                                         '端末ﾏｽﾀ
    ...
    Private mFTERM_CUT_STS As Nullable(Of Integer)                               '端末切離状態
#End Region
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ''' <summary>
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    Public Function GET_TDSP_TERM(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        If IsNull(FTERM_CUT_STS) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FTERM_CUT_STS = :" & UBound(strBindField) - 1 & " --端末切離状態")
        End If
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    Public Sub UPDATE_TDSP_TERM()
        ...
        If IsNull(mFTERM_CUT_STS) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FTERM_CUT_STS = NULL --端末切離状態")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FTERM_CUT_STS = NULL --端末切離状態")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFTERM_CUT_STS
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FTERM_CUT_STS = :" & Ubound(strBindField) - 1 & " --端末切離状態")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FTERM_CUT_STS = :" & Ubound(strBindField) - 1 & " --端末切離状態")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " WHERE")
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_TDSP_TERM()
        ...
        intCount = intCount + 1
        If IsNull(mFTERM_CUT_STS) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --端末切離状態")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --端末切離状態")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFTERM_CUT_STS
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --端末切離状態")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --端末切離状態")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " )")
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FTERM_CUT_STS")) = False Then mFTERM_CUT_STS = objObject.FTERM_CUT_STS '端末切離状態

...
''' 端末別許可ﾏｽﾀﾃｰﾌﾞﾙｸﾗｽ
''' </summary>
Public Class TBL_TDSP_PMT_TERM
...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TDSP_PMT_TERM()                                     '端末別許可ﾏｽﾀ
    Private mstrUSER_SQL As String                                               'ﾕｰｻﾞｰSQL
    Private mORDER_BY As String                                                  'OrderBy句
    Private mWHERE As String                                                     'Where句
    Private mFTERM_KUBUN As Nullable(Of Integer)                                 '端末区分
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ...
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
    ''' </summary>
    Public ReadOnly Property ARYME() As TBL_TDSP_PMT_TERM()
        Get
            Return mobjAryMe
        End Get
    End Property
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TDSP_PMT_TERM(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        strSQL.Append(vbCrLf & "    TDSP_PMT_TERM")
        ...
        If IsNull(FTERM_KUBUN) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FTERM_KUBUN = :" & UBound(strBindField) - 1 & " --端末区分")
        End If
...
#Region "  ﾃﾞｰﾀ取得(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TDSP_PMT_TERM_ANY() As RetCode
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            For ii = LBound(mobjAryMe) To UBound(mobjAryMe)
                ...
                mobjAryMe(ii) = New TBL_TDSP_PMT_TERM(Owner, objDb, objDbLog)
...
#Region "  ﾃﾞｰﾀ取得(ｶｽﾀﾑ抽出)           "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TDSP_PMT_TERM_USER(Optional ByRef objUSER_PARAM As Object(,) = Nothing) As RetCode
...
#Region "  ﾃﾞｰﾀ取得(ｶｳﾝﾄ)               "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TDSP_PMT_TERM_COUNT() As Integer
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub UPDATE_TDSP_PMT_TERM()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        ...
        '***********************
        If 1 <> 1 Then
        ElseIf IsNull(mFTERM_KUBUN) = True Then
            strMsg = ERRMSG_ERR_PROPERTY & "[端末区分]"
            Throw New UserException(strMsg)
        ...
        If IsNull(mFTERM_KUBUN) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FTERM_KUBUN = NULL --端末区分")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FTERM_KUBUN = NULL --端末区分")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFTERM_KUBUN
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FTERM_KUBUN = :" & Ubound(strBindField) - 1 & " --端末区分")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FTERM_KUBUN = :" & Ubound(strBindField) - 1 & " --端末区分")
        End If
        intCount = intCount + 1
        ...
        strSQL.Append(vbCrLf & " WHERE")
        strSQL.Append(vbCrLf & "        1 = 1 ")
        If IsNull(FTERM_KUBUN) = True Then
            strSQL.Append(vbCrLf & "    AND FTERM_KUBUN IS NULL --端末区分")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_TDSP_PMT_TERM()
        ...
        Dim intCount As Integer = 0
        If IsNull(mFTERM_KUBUN) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --端末区分")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --端末区分")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFTERM_KUBUN
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --端末区分")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --端末区分")
        End If
        intCount = intCount + 1
        If IsNull(mFDISP_ID) = True Then
...
#Region "  ﾃﾞｰﾀ削除                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_TDSP_PMT_TERM()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾃﾞｰﾀ削除(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_TDSP_PMT_TERM_ANY()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FTERM_KUBUN")) = False Then mFTERM_KUBUN = objObject.FTERM_KUBUN '端末区分
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        strMsg &= "[ﾃｰﾌﾞﾙ名:端末別許可ﾏｽﾀ]"
        If IsNotNull(FTERM_KUBUN) Then
            strMsg &= "[端末区分:" & FTERM_KUBUN & "]"
        End If
...
#Region "  ﾚｺｰﾄﾞ全取得(条件:画面ID,ｺﾝﾄﾛｰﾙID)           "
    Public Function GET_TDSP_PMT_TERM_FDISP_ID() As RetCode
        Try

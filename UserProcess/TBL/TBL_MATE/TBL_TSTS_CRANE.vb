...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TSTS_CRANE()                                        'ｸﾚｰﾝ状態
    Private mstrUSER_SQL As String                                               'ﾕｰｻﾞｰSQL
    Private mORDER_BY As String                                                  'OrderBy句
    Private mWHERE As String                                                     'Where句
    Private mFEQ_ID As String                                                    '設備ID
    Private mFCOMP_KUBUN As Nullable(Of Integer)                                 '完了区分
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    Public Function GET_TSTS_CRANE(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        If IsNull(FCOMP_KUBUN) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FCOMP_KUBUN = :" & UBound(strBindField) - 1 & " --完了区分")
        End If
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    Public Sub UPDATE_TSTS_CRANE()
        ...
        If IsNull(mFCOMP_KUBUN) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FCOMP_KUBUN = NULL --完了区分")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FCOMP_KUBUN = NULL --完了区分")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFCOMP_KUBUN
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FCOMP_KUBUN = :" & Ubound(strBindField) - 1 & " --完了区分")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FCOMP_KUBUN = :" & Ubound(strBindField) - 1 & " --完了区分")
        End If
        intCount = intCount + 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_TSTS_CRANE()
        ...
        intCount = intCount + 1
        If IsNull(mFCOMP_KUBUN) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --完了区分")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --完了区分")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFCOMP_KUBUN
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --完了区分")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --完了区分")
        End If
        intCount = intCount + 1
        If IsNull(mFDENBUN) = True Then
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FCOMP_KUBUN")) = False Then mFCOMP_KUBUN = objObject.FCOMP_KUBUN '完了区分

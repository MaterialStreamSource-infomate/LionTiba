...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TMST_AREA()                                         'ｴﾘｱﾏｽﾀ
    Private mstrUSER_SQL As String                                               'ﾕｰｻﾞｰSQL
    Private mORDER_BY As String                                                  'OrderBy句
    Private mWHERE As String                                                     'Where句
    Private mFAREA_CD As Nullable(Of Integer)                                    'ｴﾘｱｺｰﾄﾞ
    Private mFPLACE_CD As Nullable(Of Integer)                                   '保管場所ｺｰﾄﾞ
    Private mFAREA_NAME As String                                                'ｴﾘｱ名称
#End Region
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ''' <summary>
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
    ''' </summary>
    Public ReadOnly Property ARYME() As TBL_TMST_AREA()
        Get
            Return mobjAryMe
        End Get
    End Property
    ...
    ''' ｴﾘｱ名称
    ''' </summary>
    Public Property FAREA_NAME() As String
        Get
            Return mFAREA_NAME
        End Get
        Set(ByVal Value As String)
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TMST_AREA(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        strSQL.Append(vbCrLf & "    TMST_AREA")
        ...
        End If
        If IsNull(FAREA_NAME) = False Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFAREA_NAME
            strSQL.Append(vbCrLf & "    AND FAREA_NAME = :" & UBound(strBindField) - 1 & " --ｴﾘｱ名称")
        End If
...
#Region "  ﾃﾞｰﾀ取得(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TMST_AREA_ANY() As RetCode
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            For ii = LBound(mobjAryMe) To UBound(mobjAryMe)
                ...
                mobjAryMe(ii) = New TBL_TMST_AREA(Owner, objDb, objDbLog)
...
#Region "  ﾃﾞｰﾀ取得(ｶｽﾀﾑ抽出)           "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TMST_AREA_USER(Optional ByRef objUSER_PARAM As Object(,) = Nothing) As RetCode
...
#Region "  ﾃﾞｰﾀ取得(ｶｳﾝﾄ)               "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TMST_AREA_COUNT() As Integer
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    Public Sub UPDATE_TMST_AREA()
        ...
        ElseIf IsNull(mFAREA_CD) = True Then
            strMsg = ERRMSG_ERR_PROPERTY & "[ｴﾘｱｺｰﾄﾞ]"
            Throw New UserException(strMsg)
        End If
        ...
        End If
        intCount = intCount + 1
        If IsNull(mFAREA_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FAREA_NAME = NULL --ｴﾘｱ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FAREA_NAME = NULL --ｴﾘｱ名称")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFAREA_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FAREA_NAME = :" & Ubound(strBindField) - 1 & " --ｴﾘｱ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FAREA_NAME = :" & Ubound(strBindField) - 1 & " --ｴﾘｱ名称")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " WHERE")
        ...
        If IsNull(FAREA_CD) = True Then
            strSQL.Append(vbCrLf & "    AND FAREA_CD IS NULL --ｴﾘｱｺｰﾄﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_TMST_AREA()
        ...
        intCount = intCount + 1
        If IsNull(mFAREA_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --ｴﾘｱ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --ｴﾘｱ名称")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFAREA_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --ｴﾘｱ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --ｴﾘｱ名称")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " )")
...
#Region "  ﾃﾞｰﾀ削除(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_TMST_AREA_ANY()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
        ...
        If IsNotNull(FPLACE_CD) = True Then
            ...
            strSQL.Append(vbCrLf & "    AND FPLACE_CD = :" & UBound(strBindField) - 1 & " --保管場所ｺｰﾄﾞ")
        End If
        If IsNotNull(FAREA_NAME) = True Then
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFAREA_NAME
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FAREA_NAME")) = False Then mFAREA_NAME = objObject.FAREA_NAME 'ｴﾘｱ名称
...
#Region "  ﾃﾞｰﾀ→変数                   "
    ...
    Private Sub SET_DATA(ByVal objRow As DataRow)
        ...
        mFAREA_NAME = TO_STRING_NULLABLE(objRow("FAREA_NAME"))
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        End If
        If IsNotNull(FAREA_NAME) Then
            strMsg &= "[ｴﾘｱ名称:" & FAREA_NAME & "]"
        End If

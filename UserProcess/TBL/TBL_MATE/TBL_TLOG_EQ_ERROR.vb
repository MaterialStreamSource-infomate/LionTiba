...
''' 設備異常ﾛｸﾞﾃｰﾌﾞﾙｸﾗｽ
''' </summary>
Public Class TBL_TLOG_EQ_ERROR
...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TLOG_EQ_ERROR()                                     '設備異常ﾛｸﾞ
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ...
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
    ''' </summary>
    Public ReadOnly Property ARYME() As TBL_TLOG_EQ_ERROR()
        Get
            Return mobjAryMe
        End Get
    End Property
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TLOG_EQ_ERROR(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        strSQL.Append(vbCrLf & "    TLOG_EQ_ERROR")
...
#Region "  ﾃﾞｰﾀ取得(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TLOG_EQ_ERROR_ANY() As RetCode
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            For ii = LBound(mobjAryMe) To UBound(mobjAryMe)
                ...
                mobjAryMe(ii) = New TBL_TLOG_EQ_ERROR(Owner, objDb, objDbLog)
...
#Region "  ﾃﾞｰﾀ取得(ｶｽﾀﾑ抽出)           "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TLOG_EQ_ERROR_USER(Optional ByRef objUSER_PARAM As Object(,) = Nothing) As RetCode
...
#Region "  ﾃﾞｰﾀ取得(ｶｳﾝﾄ)               "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TLOG_EQ_ERROR_COUNT() As Integer
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub UPDATE_TLOG_EQ_ERROR()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
...
#Region "  ﾃﾞｰﾀ削除                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_TLOG_EQ_ERROR()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾃﾞｰﾀ削除(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_TLOG_EQ_ERROR_ANY()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#End Region
    ...
    '**********************************************************************************************
    '↓↓↓ｼｽﾃﾑ共通
#Region "  設備異常ﾛｸﾞ追加  [SEQ発番]           (Public  ADD_TLOG_EQ_ERROR_SEQ)"
    Public Sub ADD_TLOG_EQ_ERROR_SEQ()
        Try
            ...
            objTPRG_SEQNO.FSEQ_ID = FSEQ_ID_SSVRLOG_NO_EQ_ERROR              'ｼｰｹﾝｽID
    ...
    End Sub
#End Region
#Region "  設備異常ﾛｸﾞ更新  [設備ID]            (Public  UPDATE_TLOG_EQ_ERROR_FEQ_ID)"
    Public Sub UPDATE_TLOG_EQ_ERROR_FEQ_ID()
        Try
            ...
            'ﾌﾟﾛﾊﾟﾃｨﾁｪｯｸ
            '***********************
            If IsNothing(mFEQ_ID) = True Then
                strMsg = ERRMSG_ERR_PROPERTY & "[設備ID]"
                Throw New UserException(strMsg)
            End If
            If IsNothing(mFHUKKI_DT) = True Then
                strMsg = ERRMSG_ERR_PROPERTY & "[復旧日時]"
                Throw New UserException(strMsg)
            End If
            ...
            strSQL &= vbCrLf & " SET"
            strSQL &= vbCrLf & "    FHUKKI_DT = TO_DATE('" & Format(mFHUKKI_DT, "yyyy/MM/dd HH:mm:ss") & "','YYYY/MM/DD HH24:MI:SS')"
            strSQL &= vbCrLf & " WHERE"
            strSQL &= vbCrLf & "        FEQ_ID = '" & TO_STRING(mFEQ_ID) & "'"              '設備ID
            strSQL &= vbCrLf & "    AND FHUKKI_DT IS NULL"
            strSQL &= vbCrLf
            ...
            If intRetSQL < 1 Then
                ...
                '' ''strSQL = Replace(strSQL, vbCrLf, "")
                '' ''strMsg = ERRMSG_ERR_UPDATE & "(対象行無し)[ﾃｰﾌﾞﾙ:TLOG_EQ_ERROR]"
                '' ''Throw New UserException(strMsg)
            End If

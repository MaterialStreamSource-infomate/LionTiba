...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TDSP_NAME()                                         '画面ﾏｽﾀ
    Private mstrUSER_SQL As String                                               'ﾕｰｻﾞｰSQL
    Private mORDER_BY As String                                                  'OrderBy句
    Private mWHERE As String                                                     'Where句
    Private mFDISP_ID As String                                                  '画面ID
    Private mFCTRL_ID As String                                                  'ｺﾝﾄﾛｰﾙID
    Private mFOBJECT_NAME As String                                              'ｵﾌﾞｼﾞｪｸﾄ名
    Private mFPASS_CHECK_FLAG As Nullable(Of Integer)                            'ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ
#End Region
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ''' <summary>
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    Public Function GET_TDSP_NAME(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        If IsNull(FOBJECT_NAME) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FOBJECT_NAME = :" & UBound(strBindField) - 1 & " --ｵﾌﾞｼﾞｪｸﾄ名")
        End If
        If IsNull(FPASS_CHECK_FLAG) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FPASS_CHECK_FLAG = :" & UBound(strBindField) - 1 & " --ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ")
        End If
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    Public Sub UPDATE_TDSP_NAME()
        ...
        ElseIf IsNull(mFOBJECT_NAME) = True Then
            strMsg = ERRMSG_ERR_PROPERTY & "[ｵﾌﾞｼﾞｪｸﾄ名]"
            Throw New UserException(strMsg)
        End If
        ...
        If IsNull(mFOBJECT_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FOBJECT_NAME = NULL --ｵﾌﾞｼﾞｪｸﾄ名")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FOBJECT_NAME = NULL --ｵﾌﾞｼﾞｪｸﾄ名")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFOBJECT_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FOBJECT_NAME = :" & Ubound(strBindField) - 1 & " --ｵﾌﾞｼﾞｪｸﾄ名")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FOBJECT_NAME = :" & Ubound(strBindField) - 1 & " --ｵﾌﾞｼﾞｪｸﾄ名")
        End If
        intCount = intCount + 1
        If IsNull(mFPASS_CHECK_FLAG) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FPASS_CHECK_FLAG = NULL --ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FPASS_CHECK_FLAG = NULL --ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFPASS_CHECK_FLAG
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FPASS_CHECK_FLAG = :" & Ubound(strBindField) - 1 & " --ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FPASS_CHECK_FLAG = :" & Ubound(strBindField) - 1 & " --ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " WHERE")
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_TDSP_NAME()
        ...
        intCount = intCount + 1
        If IsNull(mFOBJECT_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --ｵﾌﾞｼﾞｪｸﾄ名")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --ｵﾌﾞｼﾞｪｸﾄ名")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFOBJECT_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --ｵﾌﾞｼﾞｪｸﾄ名")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --ｵﾌﾞｼﾞｪｸﾄ名")
        End If
        intCount = intCount + 1
        If IsNull(mFPASS_CHECK_FLAG) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFPASS_CHECK_FLAG
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " )")
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FOBJECT_NAME")) = False Then mFOBJECT_NAME = objObject.FOBJECT_NAME 'ｵﾌﾞｼﾞｪｸﾄ名
        If IsNothing(objType.GetProperty("FPASS_CHECK_FLAG")) = False Then mFPASS_CHECK_FLAG = objObject.FPASS_CHECK_FLAG 'ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        If IsNotNull(FOBJECT_NAME) Then
            strMsg &= "[ｵﾌﾞｼﾞｪｸﾄ名:" & FOBJECT_NAME & "]"
        End If
        If IsNotNull(FPASS_CHECK_FLAG) Then
            strMsg &= "[ﾊﾟｽﾜｰﾄﾞ認証ﾌﾗｸﾞ:" & FPASS_CHECK_FLAG & "]"
        End If

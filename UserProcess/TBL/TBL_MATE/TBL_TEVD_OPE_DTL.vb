...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TEVD_OPE_DTL()                                      '作業履歴詳細
    Private mstrUSER_SQL As String                                               'ﾕｰｻﾞｰSQL
    Private mORDER_BY As String                                                  'OrderBy句
    Private mWHERE As String                                                     'Where句
    Private mFLOG_NO As String                                                   'ﾛｸﾞ№
    Private mFORDER As Nullable(Of Integer)                                      '表示順
    Private mFDENBUN_ITEM_NAME As String                                         '電文ｱｲﾃﾑ名称
    Private mFDENBUN_ITEM_DATA As String                                         '電文ｱｲﾃﾑﾃﾞｰﾀ
#End Region
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ''' <summary>
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    Public Function GET_TEVD_OPE_DTL(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        If IsNull(FDENBUN_ITEM_NAME) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FDENBUN_ITEM_NAME = :" & UBound(strBindField) - 1 & " --電文ｱｲﾃﾑ名称")
        End If
        If IsNull(FDENBUN_ITEM_DATA) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FDENBUN_ITEM_DATA = :" & UBound(strBindField) - 1 & " --電文ｱｲﾃﾑﾃﾞｰﾀ")
        End If
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    Public Sub UPDATE_TEVD_OPE_DTL()
        ...
        ElseIf IsNull(mFORDER) = True Then
            strMsg = ERRMSG_ERR_PROPERTY & "[表示順]"
            Throw New UserException(strMsg)
        End If
        ...
        If IsNull(mFDENBUN_ITEM_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FDENBUN_ITEM_NAME = NULL --電文ｱｲﾃﾑ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FDENBUN_ITEM_NAME = NULL --電文ｱｲﾃﾑ名称")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFDENBUN_ITEM_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FDENBUN_ITEM_NAME = :" & Ubound(strBindField) - 1 & " --電文ｱｲﾃﾑ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FDENBUN_ITEM_NAME = :" & Ubound(strBindField) - 1 & " --電文ｱｲﾃﾑ名称")
        End If
        intCount = intCount + 1
        If IsNull(mFDENBUN_ITEM_DATA) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FDENBUN_ITEM_DATA = NULL --電文ｱｲﾃﾑﾃﾞｰﾀ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FDENBUN_ITEM_DATA = NULL --電文ｱｲﾃﾑﾃﾞｰﾀ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFDENBUN_ITEM_DATA
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FDENBUN_ITEM_DATA = :" & Ubound(strBindField) - 1 & " --電文ｱｲﾃﾑﾃﾞｰﾀ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FDENBUN_ITEM_DATA = :" & Ubound(strBindField) - 1 & " --電文ｱｲﾃﾑﾃﾞｰﾀ")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " WHERE")
        ...
        If IsNull(FORDER) = True Then
            strSQL.Append(vbCrLf & "    AND FORDER IS NULL --表示順")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & UBound(strBindField) - 1
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    Public Sub ADD_TEVD_OPE_DTL()
        ...
        intCount = intCount + 1
        If IsNull(mFDENBUN_ITEM_NAME) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --電文ｱｲﾃﾑ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --電文ｱｲﾃﾑ名称")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFDENBUN_ITEM_NAME
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --電文ｱｲﾃﾑ名称")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --電文ｱｲﾃﾑ名称")
        End If
        intCount = intCount + 1
        If IsNull(mFDENBUN_ITEM_DATA) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --電文ｱｲﾃﾑﾃﾞｰﾀ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --電文ｱｲﾃﾑﾃﾞｰﾀ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFDENBUN_ITEM_DATA
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --電文ｱｲﾃﾑﾃﾞｰﾀ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --電文ｱｲﾃﾑﾃﾞｰﾀ")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " )")
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FDENBUN_ITEM_NAME")) = False Then mFDENBUN_ITEM_NAME = objObject.FDENBUN_ITEM_NAME '電文ｱｲﾃﾑ名称
        If IsNothing(objType.GetProperty("FDENBUN_ITEM_DATA")) = False Then mFDENBUN_ITEM_DATA = objObject.FDENBUN_ITEM_DATA '電文ｱｲﾃﾑﾃﾞｰﾀ
...
#Region "  ｴﾗｰﾒｯｾｰｼﾞ文字列作成01        "
    ...
    Private Sub MAKE_ERRMSG01(ByRef strMsg As String)
        ...
        If IsNotNull(FDENBUN_ITEM_NAME) Then
            strMsg &= "[電文ｱｲﾃﾑ名称:" & FDENBUN_ITEM_NAME & "]"
        End If
        If IsNotNull(FDENBUN_ITEM_DATA) Then
            strMsg &= "[電文ｱｲﾃﾑﾃﾞｰﾀ:" & FDENBUN_ITEM_DATA & "]"
        End If
...
#End Region
    ...
    '**********************************************************************************************
    '↓↓↓ｼｽﾃﾑ共通
#Region "  ﾃﾞｰﾀ削除(ｺﾞﾐ)                (Public  DELETE_TEVD_OPE_DTL_SCRAP)"

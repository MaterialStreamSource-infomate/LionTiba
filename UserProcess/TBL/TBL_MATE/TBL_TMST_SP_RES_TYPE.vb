...
''' 引当ﾀｲﾌﾟ特定ﾏｽﾀﾃｰﾌﾞﾙｸﾗｽ
''' </summary>
Public Class TBL_TMST_SP_RES_TYPE
...
#Region "  ｸﾗｽ変数定義                  "
    'ﾌﾟﾛﾊﾟﾃｨ
    Private mobjAryMe As TBL_TMST_SP_RES_TYPE()                                  '引当ﾀｲﾌﾟ特定ﾏｽﾀ
...
#Region "  ﾌﾟﾛﾊﾟﾃｨ定義                  "
    ...
    ''' ｼｽﾃﾑ変数 (自ｸﾗｽ型配列)
    ''' </summary>
    Public ReadOnly Property ARYME() As TBL_TMST_SP_RES_TYPE()
        Get
            Return mobjAryMe
        End Get
    End Property
...
#Region "  ﾃﾞｰﾀ取得                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TMST_SP_RES_TYPE(Optional ByVal blnNotFoundError As Boolean = True) As RetCode
        ...
        strSQL.Append(vbCrLf & "    TMST_SP_RES_TYPE")
        ...
        If IsNull(FRES_TYPE) = False Then
            ...
            strSQL.Append(vbCrLf & "    AND FRES_TYPE = :" & UBound(strBindField) - 1 & " --引当ﾀｲﾌﾟ")
        End If
        ...
        objDataSet.Clear()
        strDataSetName = "TMST_SP_RES_TYPE"
...
#Region "  ﾃﾞｰﾀ取得(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TMST_SP_RES_TYPE_ANY() As RetCode
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
        ...
        If objDataSet.Tables(strDataSetName).Rows.Count > 0 Then
            ...
            For ii = LBound(mobjAryMe) To UBound(mobjAryMe)
                ...
                mobjAryMe(ii) = New TBL_TMST_SP_RES_TYPE(Owner, objDb, objDbLog)
...
#Region "  ﾃﾞｰﾀ取得(ｶｽﾀﾑ抽出)           "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TMST_SP_RES_TYPE_USER(Optional ByRef objUSER_PARAM As Object(,) = Nothing) As RetCode
...
#Region "  ﾃﾞｰﾀ取得(ｶｳﾝﾄ)               "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Function GET_TMST_SP_RES_TYPE_COUNT() As Integer
        Dim strSQL As New StringBuilder 'SQL文
        Dim objDataSet As New DataSet   'ﾃﾞｰﾀｾｯﾄ
        Dim strDataSetName As String    'ﾃﾞｰﾀｾｯﾄ名
        Dim objRow As DataRow           '1ﾚｺｰﾄﾞ分のﾃﾞｰﾀ
...
#Region "  ﾃﾞｰﾀ更新                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub UPDATE_TMST_SP_RES_TYPE()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        ...
        If IsNull(mFRES_TYPE) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FRES_TYPE = NULL --引当ﾀｲﾌﾟ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FRES_TYPE = NULL --引当ﾀｲﾌﾟ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFRES_TYPE
            If intCount = 0 Then strSQL.Append(vbCrLf & "    FRES_TYPE = :" & Ubound(strBindField) - 1 & " --引当ﾀｲﾌﾟ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,FRES_TYPE = :" & Ubound(strBindField) - 1 & " --引当ﾀｲﾌﾟ")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " WHERE")
...
#Region "  ﾃﾞｰﾀ追加                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub ADD_TMST_SP_RES_TYPE()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
        ...
        intCount = intCount + 1
        If IsNull(mFRES_TYPE) = True Then
            If intCount = 0 Then strSQL.Append(vbCrLf & "    NULL --引当ﾀｲﾌﾟ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,NULL --引当ﾀｲﾌﾟ")
        Else
            ReDim Preserve strBindField(UBound(strBindField) + 1)
            ReDim Preserve objBindValue(UBound(objBindValue) + 1)
            strBindField(UBound(strBindField)) = ":" & Ubound(strBindField) - 1
            objBindValue(UBound(objBindValue)) = mFRES_TYPE
            If intCount = 0 Then strSQL.Append(vbCrLf & "    :" & Ubound(strBindField) - 1 & " --引当ﾀｲﾌﾟ")
            If intCount > 0 Then strSQL.Append(vbCrLf & "   ,:" & Ubound(strBindField) - 1 & " --引当ﾀｲﾌﾟ")
        End If
        intCount = intCount + 1
        strSQL.Append(vbCrLf & " )")
...
#Region "  ﾃﾞｰﾀ削除                     "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_TMST_SP_RES_TYPE()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾃﾞｰﾀ削除(複数ﾚｺｰﾄﾞ)          "
    ...
    ''' <remarks></remarks>
    '''**********************************************************************************************
    Public Sub DELETE_TMST_SP_RES_TYPE_ANY()
        Dim strSQL As New StringBuilder     'SQL文
        Dim strMsg As String                'ﾒｯｾｰｼﾞ
        Dim intRetSQL As Integer            'SQL実行戻り値
        Dim objParameter(1, 0) As Object
...
#Region "  ﾌﾟﾛﾊﾟﾃｨｺﾋﾟｰ                  "
    Public Sub COPY_PROPERTY(ByVal objObject As Object)
        ...
        If IsNothing(objType.GetProperty("FRES_TYPE")) = False Then mFRES_TYPE = objObject.FRES_TYPE '引当ﾀｲﾌﾟ
